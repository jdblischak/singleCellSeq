...
**Version using molecule counts corrected for collision probability**
...
molecules_single_prev <- molecules_single
molecules_single <- -1024 * log(1 - molecules_single / 1024)
stopifnot(sum(is.na(molecules_single)) == sum(molecules_single_prev > 1024))
rows_w_na <- apply(molecules_single, 1, function(x) any(is.na(x)))
molecules_single <- molecules_single[!rows_w_na, ]
molecules_bulk <- molecules_bulk[!rows_w_na, ]
molecules <- molecules[!rows_w_na, ]
# Round so that they are still integer counts
molecules_single <- round(molecules_single)
```

```{r plot-collision-correction}
plot(molecules_single_prev[!rows_w_na, 1], molecules_single[, 1])
abline(0, 1, col = "red")
```

```{r plot-collision-correction-log2}
plot(log2(molecules_single_prev[!rows_w_na, 1]), log2(molecules_single[, 1]))
abline(0, 1, col = "red")
```
...
```{r pca-bulk-quant}
bulk_quant <- bulk_quant[gene_rows, ]
bulk_quant <- bulk_quant[apply(bulk_quant, 1, var) > 0, ]
pca_bulk_quant <- run_pca(bulk_quant)
