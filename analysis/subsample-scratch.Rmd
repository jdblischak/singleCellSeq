...
At lower number of sampled cells, reads appear tp perform better with lower sequencing.
...
It's not due to changing number of available cells because I the highest sequencing depth I used was 1.5 million.

I haven't correctly formatted the output for when there are not enough cells to be subsampled, so I removed these.

```
$ cat subsampling-results.txt | awk '$9 > 0' > subsampling-results4.txt
```

```{r}
d <- read.table("/mnt/gluster/home/jdblischak/ssd/subsampled/subsampling-results4.txt",
...
d_plot
d_plot %+% d_grouped[d_grouped$depth %in% c(250000, 1500000), ]
```
...
Available cells - don't change across depth because purposely only sampling 1.5 million reads at greatest depth.
