...
1. __CV-mean relationship__: The pattern among the expressed cells was different from the usual concave pattern we observe in scRNA-seq data when all cells are included or in  bulk RNA-seq data. Specifically, CV increases at low gene mean expression values and after some value of mean gene expression (quite low), it increases again.

2. __Overlaps of individual top CV genes__: Similar to when including non-expressed cells, there are few common top genes across individuals (~100 genes). This suggests possible individual differences in expression variablity. 

3. __Non-detected cells__: Overall, about 30% of the genes in each iPSC cell line had 50 percent or more non-detected cells. The number of detected cells is assoicated with the dynamic range of gene expression. Small number of detected cells limits the dynamic range of gene expression, which in turns associates with low expression level and limited variation. 

4. __Differential testing__: We found 680 genes with differential variation across the expressed cell. These 
...
## Differential testing - permutation
