...
drop_out$all <- rowMeans(as.matrix(molecules_filter_df) == 0)
```

```{r}
par(mfrow = c(2,2))
hist(drop_out$NA19098, breaks = 60)
hist(drop_out$NA19101, breaks = 60)
hist(drop_out$NA19239, breaks = 60)

par(mfrow = c(2,2))
plot(drop_out$NA19098, drop_out$NA19101,
     xlab = "NA19098", ylab = "NA19101",
     pch = 16, cex = .5, col = scales::alpha("black", .8))
abline(0, 1, col = "blue")

plot(drop_out$NA19098, drop_out$NA19239,
      xlab = "NA19098", ylab = "NA19239",
     pch = 16, cex = .5, col = scales::alpha("black", .8))
abline(0, 1, col = "blue")

plot(drop_out$NA19101, drop_out$NA19239,
      xlab = "NA19101", ylab = "NA19101",
     pch = 16, cex = .5, col = scales::alpha("black", .8))
abline(0, 1, col = "blue")
```
...
## Density of those with high average CV 

```{r}
gene_elect <- which(
  rank(rowMeans(cbind(ENSG_cv_adj$NA19098$log10cv2_adj,
                      ENSG_cv_adj$NA19101$log10cv2_adj,
                      ENSG_cv_adj$NA19239$log10cv2_adj))) > NROW(ENSG_cv_adj$NA19098) - 20)
gene_elect <- rownames(molecules_final)[gene_elect]
#rank(mad_expressed)
...
plot_density_overlay(
  ...
  individual_label = NULL,
  which_gene = gene_elect[i], 
  labels = paste(rank(mad_expressed)[which(rownames(molecules_final_df) %in% gene_elect[i])], "(", length(mad_expressed), ")", sep = ""),
  xlims = c(1,14), ylims = NULL,
  gene_symbols = gene_symbols)
plot_density_overlay(
  molecules = molecules_final_df,
...
pdf("figure/cv-adjusted-summary-pois-expressed.Rmd/density-high-all-cv.pdf",
    height = 12, width =8)
par(mfrow = c(5,4))
for (i in 1:length(gene_elect)) {
plot_density_overlay(
