...
Some genes are assigned a count of 1 across all the cells.
These invariant genes must be removed before performing PCA.

```{r bulk-ruv-invariant-genes}
# Variance = 0
sum(apply(bulk_ruv, 1, var) == 0)
# All assigned a count of 1
all(bulk_ruv[apply(bulk_ruv, 1, var) == 0, ] == 1)
```
...
The single cell data has no invariant genes:

```{r single-ruv-invariant-genes}
# Variance = 0
sum(apply(single_ruv, 1, var) == 0)
```

```{r pca-single-ruv}
pca_single_ruv <- run_pca(single_ruv)
