...
molecules_bulk_cpm <- cpm(molecules_bulk,
...
cor(rowMeans(molecules_single_cpm[ercc_rows, ]), ercc$conc_mix1)
```
...
cor(rowMeans(molecules_bulk_cpm[ercc_rows, ]), ercc$conc_mix1)
```
...
for (i in 1:ncol(molecules_single_cpm)) {
  fit <- lm(ercc$conc_mix1 ~ molecules_single_cpm[ercc_rows, i])
...
for (i in 1:ncol(bulk_norm)) {
  bulk_fit <- lm(ercc$conc_mix1 ~ molecules_bulk_cpm[ercc_rows, i])
...
for (i in 1:ncol(single_norm)) {
  single_fit <- lm(ercc$conc_mix1 ~ molecules_single_cpm[ercc_rows, i])
...
```{r pca-single-ruv}
pca_single_ruv <- run_pca(single_ruv[gene_rows, ])
