...
cor(rowMeans(molecules_single[ercc_rows, ]), ercc$conc_mix1)
plot(rowMeans(molecules_single[ercc_rows, ]), ercc$conc_mix1)
ercc_fit <- lm(ercc$conc_mix1 ~ rowMeans(molecules_single[ercc_rows, ]))
...
cor(rowMeans(molecules_bulk[ercc_rows, ]), ercc$conc_mix1)
```

What is the correlation of the mean molecule counts per million in the single cells to the expected concentrations?

```{r ercc-correlation-mean-cpm-single}
cor(rowMeans(molecules_single_cpm[ercc_rows, ]), log2(ercc$conc_mix1))
```

What is the correlation of the mean molecule counts per million in the bulk samples to the expected concentrations?

```{r ercc-correlation-mean-cpm-bulk}
cor(rowMeans(molecules_bulk_cpm[ercc_rows, ]), log2(ercc$conc_mix1))
```

How much variation is there in the correlation between the samples?
The following analysis uses the cpm in single cells.
...
for (i in 1:ncol(molecules_single_cpm)) {
  fit <- lm(log2(ercc$conc_mix1) ~ molecules_single_cpm[ercc_rows, i])
...
for (i in 1:ncol(bulk_norm)) {
  bulk_fit <- lm(log2(ercc$conc_mix1) ~ molecules_bulk_cpm[ercc_rows, i])
  bulk_norm[, i] <- molecules_bulk_cpm[, i] * bulk_fit$coefficients[2] +
...
for (i in 1:ncol(single_norm)) {
  single_fit <- lm(log2(ercc$conc_mix1) ~ molecules_single_cpm[ercc_rows, i])
  single_norm[, i] <- molecules_single_cpm[, i] * single_fit$coefficients[2] +
...
```{r bulk-ruv}
bulk_ruv_object <- RUVg(x = as.matrix(molecules_bulk), cIdx = ercc_rows, k = 1)
bulk_ruv <- bulk_ruv_object$normalizedCounts
bulk_ruv_cpm <- cpm(bulk_ruv, log = TRUE,
                    lib.size = calcNormFactors(bulk_ruv) * colSums(bulk_ruv))
```

```{r single-ruv}
single_ruv_object <- RUVg(x = as.matrix(molecules_single), cIdx = ercc_rows, k = 1)
single_ruv <- single_ruv_object$normalizedCounts
single_ruv_cpm <- cpm(single_ruv, log = TRUE,
                      lib.size = calcNormFactors(single_ruv) * colSums(single_ruv))
```
...
```{r pca-bulk-ruv}
pca_bulk_ruv <- run_pca(bulk_ruv_cpm[gene_rows, ])
...
```{r pca-single-ruv}
pca_single_ruv <- run_pca(single_ruv_cpm[gene_rows, ])
