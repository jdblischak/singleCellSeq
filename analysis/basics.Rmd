...
This was run on Ubuntu.
A dependency, RccpArmadillo, cannot be installed on the cluster.
Our systems admin has not been able to figure out the reason.
...
phi_molecule_plot <- ggplot(phi, aes(x = total_count, y = Phi)) +
  ...
  labs(x = "Total molecule count",
       title = "Phi measures total molecule count differences")
phi_molecule_plot
```
...
phi_read_plot <- phi_molecule_plot %+% phi %+% aes(x = total_read_count) +
  ...
  labs(x = "Total read count (x10^6)",
       title = "Phi measure total read count differences")
phi_read_plot
```

## Capture efficiency

S is supposed to measure the capture efficiency.
...
This does not appear to be estimated very well.
It may need more iterations, but it already takes ~5 hours to run the 4000 iterations.

```{r s-v-capture-efficiency}
ggplot(s, aes(x = efficiency, y = S)) +
  geom_point() +
  geom_errorbar(aes(ymin = lower, ymax = upper), alpha = 0.5) +
  geom_smooth() +
  labs(x = "ERCC capture efficiency",
       title = "S poorly models differences in capture efficiency")
```
