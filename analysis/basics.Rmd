...
Unfortunately this time we also included some cells from replicate 2 of individual NA19098.
Because it is such an outlier in its ERCC counts, it breaks the assumptions of the model.
I'll run it again with all cells from this batch removed (results will be ready in ~60 hours)000.
...
As expected, batch 19098.r2 is an outlier because its ERCC spike-ins are so much higher than all the other batches.

```{r batch-theta}
colnames(chain_theta) <- unique(anno_filter$batch)
theta_long <- gather(as.data.frame(chain_theta), key = "batch", value = "theta")
...
The low values are from replicate 2 of individual 19098.

```{r phi-versus-read-count-per-batch}
phi_read_plot + facet_grid(replicate ~ individual)
```
...
The outlier replicate 2 of individual 19098 appears to have adversely affected the modeling of S for the other batches.

```{r s-v-capture-efficiency-per-batch}
s_plot %+% s[s$batch != "NA19098.r2", ] +
