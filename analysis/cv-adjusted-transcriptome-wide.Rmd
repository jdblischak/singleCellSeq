...
We normalized CV to account for heterogeneity of per gene coefficient variation as a function of mean gene expression, using the algorithm described in [Kolodziejczk et al. (2015)](link1). Then, we estimated the first principal component loadings of samples from each of the three individuals. These three sets of PC1 gene loadsings are compared using Friedman rank sum test. 

Before normalization, the coefficients of variation significantly differ between individuals (p < 1e-5); but, after normalization, the CVs are similar between the three individuals.
