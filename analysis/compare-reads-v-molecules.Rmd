...
set.seed(12345)
sub_indices <- sample(1:nrow(reads), 250)
reads_sub <- reads[sub_indices, ]
reads_long <- gather(reads_sub, key = "id", value = "reads")
molecules_sub <- molecules[sub_indices, ]
molecules_long <- gather(molecules_sub, key = "id", value = "molecules")
reads_long <- separate(reads_long, col = "id",  sep = "\\.", remove = FALSE,
                       into = c("individual", "batch", "well"))
stopifnot(reads_long$id == molecules_long$id)
conversion <- cbind(reads_long, molecules_long)
stopifnot(nrow(conversion) == nrow(molecules_long))
dim(conversion)
head(conversion)
```

```{r convert-reads-molecules-per-batch, fig.width=9, fig.height=9}
ggplot(conversion[conversion$well != "bulk", ],
       aes(x = reads, y = molecules, col = individual,
           shape = as.factor(batch))) +
  geom_point() +
  ...
  labs(x = "Total number of reads",
       y = "Total number of molecules",
       title = "Conversion per batch (subsample of 250 genes)")
```
