...
load("rda/cv-adjusted-summary-pois-expressed/mad-perm-values.rda")
...
names(perm_pval) <- rownames(expressed_dm)
```
...
permuted_pval_subset <- perm_pval[which(names(perm_pval) %in% genes_included)]
names(permuted_pval_subset) <- names(perm_pval)[which(names(perm_pval) %in% genes_included)]
...
# for each bin
permuted_pval_subset[which(permuted_pval_subset == 0)] <- 1/length(perm_pval)
mad_expressed <- rowMedians( abs( as.matrix(expressed_dm_subset) - rowMedians(as.matrix(expressed_dm_subset)) ) )
