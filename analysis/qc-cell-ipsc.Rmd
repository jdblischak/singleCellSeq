...
ggplot(summary_per_sample_reads_single_qc, aes(x = total.mapped, y = adj.ERCC.ratios, col = as.factor(individual), label = as.character(cell_number))) + geom_text(fontface=3) + facet_grid(individual ~ batch) + theme(axis.text.x = element_text(angle = 90, hjust = 0.9, vjust = 0.5))
```
...
pca_reads_cells_anno$qc_filter <- pca_reads_cells_anno$cell_filter &
...
table(pca_reads_cells_anno$cell_filter, pca_reads_cells_anno$total_cutoff,
      dnn = c("Num cells == 1", "Total reads > 2e6"))
```
