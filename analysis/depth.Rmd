...
```{r sequencing-depth-num-cells, fig.width=10, fig.height=10}
...
On the other hand, utilizing the UMIs, we do see a correlation with the number of molecules per well with the number of cells per well.
Crucially, in accoradance with our expecations, this trend is not present in the ERCC molecules.
...
Furthermore, the increasing trend is also observed with the number of observed genes.

```{r molecules-num-cells, fig.width=10, fig.height=10}
...
We do observe this expected trend with the ERCC molecules, and to a lesser extent with the ERCC reads.
...
The percentage of unmapped reads, another quality measure, does decrease from one cell to two cells, but does not change much from 2 cells to 3 cells.
...
```{r perc-ercc-num-cells, fig.width=10, fig.height=10}
...
We do observe the latter, but not the former.
More specifically, for wells that contain one cell, we observe an increase in the number of gene molecules with an increase in concentration.
However, the concentration decreases with increasing number of cells per well.

```{r concentration-num-cells, fig.width=10, fig.height=10}
plot_conc_molecules_gene <- ggplot(anno_single, aes(x = concentration, y = total_molecules_gene / 10^3,
  ...
  labs(x = "Concentration", y = "", title = "Gene molecules") +
...
plot_grid(plot_conc_molecules_gene, plot_cell_concentration, plot_conc_molecules_gene_1cell,
          labels = LETTERS[1:3], nrow = 3)
```
...
`r quantile(anno_single$total_molecules_gene[anno_single$cell_number == 1], probs = 0.25)`,
...
```{r molecules-num-cells-batch, fig.width=10, fig.height=10}
plot_conc_molecules_gene_batch <- ggplot(anno_single[anno_single$cell_number == 1, ],
  ...
  scale_color_brewer(palette = "Dark2") 
plot_grid(plot_cell_mol_gene %+% aes(fill = batch) + theme(legend.position = "none"),
          plot_cell_mol_ercc %+% aes(fill = batch) + theme(legend.position = "none"),
          plot_cell_num_genes %+% aes(fill = batch) + theme(legend.position = "bottom"),
...
```{r ercc-metrics, fig.width=10, fig.height=10}
