...
```{r}
anno_single_long <- gather(anno_single, key = "metric", value = "value",
                           total_reads_gene, total_reads_ercc, total_reads,
                           total_molecules_gene, total_molecules_ercc, total_molecules,
                           total_mapped, total_unmapped, depth, concentration,
                           perc_ercc_reads, perc_ercc_molecules, perc_unmapped,
                           num_genes)
head(anno_single_long)
```

```{r}
plot_cell_num <- ggplot(anno_single_long[anno_single_long$cell_number %in% 0:3, ],
                        aes(x = as.factor(cell_number), y = value, fill = individual)) +
  geom_boxplot() +
...
```{r}
ggplot(anno_single, aes(x = total_reads_gene, y = total_molecules_gene)) +
  geom_point(aes(color = individual, shape = replicate)) +
  facet_wrap(~cell_number)
```
...
```{r concentration-total-molecule-gene}
ggplot(anno_single,
       aes(x = concentration, y = total_molecules_gene)) +
  geom_text(aes(label = cell_number))
```

```{r concentration-total-molecule-gene-one-cell}
ggplot(anno_single[anno_single$cell_number == 1, ],
       aes(x = concentration, y = total_molecules_gene)) +
  geom_text(aes(label = individual))
```

Investigates suspicious cells.

```{r}
outliers <- anno_single %>% filter(cell_number == 1, concentration < 1.5,
                                   total_molecules_gene > 140000)
write.table(outliers, file = "../data/outlier-single-cells.txt",
            sep = "\t", row.names = FALSE, quote = FALSE)
```

```{r}
plot_cell_num %+% anno_single_long[!(anno_single_long$sample_id %in% outliers$sample_id), ]
```
