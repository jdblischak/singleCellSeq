...
ercc_index <- grep("ERCC", rownames(molecules))
length(ercc_index)

efficiency <- numeric(length = ncol(molecules))
total_ercc_molecules <- sum(ercc_plot$expected)
for (i in 1:ncol(molecules)) {
  efficiency[i] <- sum(molecules[ercc_index, i]) / total_ercc_molecules
}
summary(efficiency)
```
