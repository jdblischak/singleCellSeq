...
Because the differential analysis of coefficient of variation is performed on the data set that has been cleaned up batch effect, we suspect that there is little variation between the individual replicates. We subsequently perform subsampling for each individual to generate samples of arbitary sizes; for example, we combine the two biological replicates of NA19098 and then take random samples of size 20 with replacement for 10 times. As a result, we are able to obtain a sufficiently large number of samples for each individual.
