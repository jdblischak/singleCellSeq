...
plot(pca_single_cells$perc_explained)
...
ggplot(pca_single_cells_anno, aes(x = PC1, y = PC2, col = as.factor(individual), shape = as.factor(batch))) +geom_point() + labs(title="pluripotency genes (cpm)")
```
...
pca_cpm_ERCC <- prcomp(t(molecules_single_cpm_ERCC), retx = TRUE, scale. = TRUE, center = TRUE)
plot(pca_cpm_ERCC)

pca_cpm_ERCC$perc_explained <- pca_cpm_ERCC$sdev^2 / sum(pca_cpm_ERCC$sdev^2) * 100
plot(pca_cpm_ERCC$perc_explained)

stopifnot(colnames(pca_cpm_ERCC) == 
            paste(paste0("NA", anno_single$individual),
                   anno_single$batch,
                   anno_single$well, sep = "."))

pca_cpm_ERCC_anno <- cbind(anno_single, pca_cpm_ERCC$x)
ggplot(pca_cpm_ERCC_anno, aes(x = PC1, y = PC2, col = as.factor(individual), shape = as.factor(batch))) +geom_point() + labs(title="ERCC spike-in (cpm)")

ggplot(pca_cpm_ERCC_anno, aes(x = PC1, y = PC2, col = as.factor(individual), shape = as.factor(batch))) +geom_point() + scale_x_continuous(limits=c(-10, 12))
```
...
pca_random <- prcomp(t(molecules_single_random), retx = TRUE, scale. = TRUE, center = TRUE)
plot(pca_random)

pca_random$perc_explained <- pca_random$sdev^2 / sum(pca_random$sdev^2) * 100
plot(pca_random$perc_explained)

stopifnot(colnames(pca_random) == 
            paste(paste0("NA", anno_single$individual),
                   anno_single$batch,
                   anno_single$well, sep = "."))

pca_random_anno <- cbind(anno_single, pca_random$x)
ggplot(pca_random_anno, aes(x = PC1, y = PC2, col = as.factor(individual), shape = as.factor(batch))) +geom_point() + labs(title="random endogenous genes (cpm)")
```
