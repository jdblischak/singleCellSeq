...
Legend: (A) Histogram of empirical p-values based on 12,192 permutations. (B) -log10 empirical p-values are plotted against average gene expression levels. Gene are sorted based on average gene expression of all cells across individuals, and then classifed into 10 equally spaced bins based on average gene expression. For each bin, we computed and displayed quantiles of -log10 p-values across genes. (C) We used the method in (C) and plotted quantiles of Median of Absolute Deviation (MAD) of genes in each of the 10 categories. 
...
plot_grid(
  ...
  ggplot( data.frame(pvals = permuted_pval$mad_pval,
         ...
         aes(x = factor(bins), y = -log10(pvals),
             fill = factor(bins)) ) +
