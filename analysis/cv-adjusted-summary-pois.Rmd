...
```{r}
go_top_shared <- read.table("../data/go-cpdb-top-1000-shared.tab",
                         sep = "\t",
                         header = TRUE)
...
```{r}
go_sig <- read.table("../data/go-cpdb-sig-dm.tab",
                         sep = "\t",
                         header = TRUE)
...
```{r}
library(mygene)
go_genes <- getGenes(gsub(";", ",",
                    as.character(go_sig$members_input_overlap_geneids)))
go_genes <- go_genes[!duplicated(go_genes[ , "symbol"]), ]
kable(data.frame(symbol = go_genes[ ,"symbol"],
                 name = go_genes[,"name"]) )
```
...
```{r}
go_overlap <- read.table("../data/go-cpdb-sig-dm.tab",
                         sep = "\t", header = TRUE,
                         stringsAsFactors = FALSE)
