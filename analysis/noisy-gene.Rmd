...
prep_molecules.cv.mean <- function(data.in,adj.19098.batch2=0){
  ...
  if(adj.19098.batch2 == 2){
	...
	# add mean
	molecules_single_qc_w_mean_cv$mean <- apply(molecules_single_qc, 1, function(x) mean(x,na.rm=TRUE) )
	# add CV
	molecules_single_qc_w_mean_cv$CV <- apply(molecules_single_qc, 1, function(x) sd(x,na.rm=TRUE) )/ apply(molecules_single_qc, 1, function(x) mean(x,na.rm=TRUE))
...
molecules_single_qc_expressed 		<- prep_molecules.cv.mean(data.in=molecules,adj.19098.batch2=0)
molecules_single_qc_expressed_adj		<- prep_molecules.cv.mean(data.in=molecules,adj.19098.batch2=1)
molecules_single_qc_expressed_rm 		<- prep_molecules.cv.mean(data.in=molecules,adj.19098.batch2=2)
molecules_single_qc_expressed_rm_ERCC 		<- prep_molecules.cv.mean(data.in=molecules,adj.19098.batch2=3)
...
plot.cv.and.mean <- function(data.in){
	...
	ggplot(data.in, aes(x = mean, y = CV, col = ERCC))  + geom_point(size = 2, alpha = 0.5) +  stat_function(fun= poisson.c, col= "#CC79A7") + stat_function(fun= four.sd, col= "#F0E442") + stat_function(fun= lossy.posson, col= "#56B4E9") + scale_x_log10() + ylim(0, 30) + scale_colour_manual(values=cbPalette) 
...
#  using the rm_ERCC
count.index  <- (!is.na(molecules_single_qc_expressed_rm_ERCC$mean))&(molecules_single_qc_expressed_rm_ERCC$mean>1)
condi.index <- (molecules_single_qc_expressed_rm_ERCC$CV > 4*(molecules_single_qc_expressed_rm_ERCC$mean^(0.5))/molecules_single_qc_expressed_rm_ERCC$mean)

sum(count.index&condi.index)

molecules_single_qc_expressed_rm_ERCC$mean[count.index&condi.index]
rownames(molecules_single_qc_expressed_rm_ERCC)[count.index&condi.index]
```
