...
### two parameters  
### 1. data.in : input data of molecules
...
prep_molecules.cv.mean <- function(data.in,adj.19098.batch2=0){
	if(adj.19098.batch2 == 1){
		#### find out which columns belong 19098 batch2
		f <- names(data.in)
		target.column <- f[grep("19098.2",f)]
  ...
  if(adj.19098.batch2 == 2){
		...
		f <- names(data.in)
		#### remove 19098.2
		data.in<- data.in[,!grepl("19098.2",f)]
...
molecules_single_qc_expressed 		<- prep_molecules.cv.mean(data.in=molecules_single_qc,adj.19098.batch2=0)
molecules_single_qc_expressed_adj		<- prep_molecules.cv.mean(data.in=molecules_single_qc,adj.19098.batch2=1)
molecules_single_qc_expressed_rm 		<- prep_molecules.cv.mean(data.in=molecules_single_qc,adj.19098.batch2=2)
molecules_single_qc_expressed_rm_ERCC 		<- prep_molecules.cv.mean(data.in=molecules_single_qc,adj.19098.batch2=3)
...
## 19098
individual <- names(molecules_single_qc)
individual_19098 <- molecules_single_qc[,grepl("19098",individual)]
individual_19098_mean_CV <- prep_molecules.cv.mean(data.in=individual_19098,adj.19098.batch2 = 3)
...
## 19101
individual_19101 <- molecules_single_qc[,grepl("19101",individual)]
individual_19101_mean_CV <- prep_molecules.cv.mean(data.in=individual_19101,adj.19098.batch2 = 0)
...
## 19239
individual_19239 <- molecules_single_qc[,grepl("19239",individual)]
individual_19239_mean_CV <- prep_molecules.cv.mean(data.in=individual_19239,adj.19098.batch2 = 0)
...
plot.cv.and.mean <- function(data.in){
	...
	ggplot(data.in, aes(x = mean, y = CV, col = ERCC))  + geom_point(size = 2, alpha = 0.5) +  stat_function(fun= poisson.c, col= "#CC79A7") + stat_function(fun= four.sd, col= "#F0E442") + stat_function(fun= lossy.posson, col= "#56B4E9") + scale_x_log10() + ylim(0, 15) + scale_colour_manual(values=cbPalette) + xlab("Average number of molecules") + ylab ("coefficient of variation (CV)")
...
noisy_gene(data.in = individual_19098_mean_CV)
noisy_gene(data.in = individual_19101_mean_CV)
noisy_gene(data.in = individual_19239_mean_CV)
