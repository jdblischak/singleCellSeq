...
prep_molecules.cv.mean <- function(data.in,adj.19098.batch2=FALSE){
	if(adj.19098.batch2){
...
molecules_single_qc_expressed 		<- prep_molecules.cv.mean(data.in=molecules,adj.19098.batch2=FALSE)
molecules_single_qc_expressed_adj		<- prep_molecules.cv.mean(data.in=molecules,adj.19098.batch2=TRUE)
...
ggplot(molecules_single_qc_expressed_adj, aes(x = mean, y = CV, col = ERCC))  + geom_point(size = 2, alpha = 0.5) + scale_x_log10() + scale_colour_manual(values=cbPalette)
