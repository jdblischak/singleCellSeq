...
The three C1 replicates of each individual were collected on different dates using cells of different passage numbers (Sup Table. 1), and hence they were also biological replicates. 
...
Unfortunately, the second C1 collection of individual 19098 contained apparently higher ERCC contents compared to all the others (Sup Figure. 2A and B), suggesting that there might be a pippetting error of lysis mix preparation which led to excess ERCC molecule counts.
To avoid potential unidnetified issues caused by the error, we decided to exclude this C1 collection from here on.
We then defined the cutoffs of the total mapped reads, the percentage of unmapped reads, the percentage of ERCC spike-in reads, and the number of genes detected by the 95 percentile of the 18 no-cell libraries (Sup Figure. 2C-F).  
Next, we identified and further removed several one-cell libraries that had high possibility to be multiple-cell libraries based on the number of total molecules, the concentration of cDNA amplicons, and the read-to-molecule conversion efficiency defined as the number of total molecules divided by the number of total reads (Sup Figure. 3).
...
While the observed levels of ERCC spike-ins strongly correlated with the known input quantities (r = 0.9932), the capture efficiency of ERCC spike-ins, defined as the fraction of total input molecules being successfully detected in each high quality cells, has the average of 9.119 %.
...
We thus decided not to explicitly  estimate the technical variability and cellular content in this study. 
...
We didn't observe any obvious confounder that need to be corrected while carrying out quality control at the level of sequencing lane.
...
The subsampling results from these libraries showed that with the sequencing depth of 50 million reads, the numbers of endogenous genes started to plateau, whereas the total molecule counts were still increasing, suggesting that we have not yet reach the saturation point of molecule counts. 
...
Thus, after removing genes with lower expression level in high quality cells, we first computed the molecule number of each gene by applying a mathematical correction to account for the effect of random counting statistics termed collision probability [@Fu2011]. 
This step also removed the genes with molecule counts larger than 1024, which in our case were all mitochondrial genes, because of the alternative transcriptional start sites. 

#### Standardization (counts-per-million)
We then checked if the sequencing depth played a role in sample total molecule counts and read counts. We found that the sample total read count were highly correlated with not only the total molecule counts but also the first principal component of overall gene expression. 
Not surprisingly, same results were also observed when we investigated the effects of sequencing depth on total molecule counts of the endogeneous genes and ERCC spike-ins.
Therefore, we standardized sample molecule counts separately for endogenous genes and ERCC spike-ins within each sample using the unit of cpm (counts-per-million). 

#### Poission regression of ERCC counts

#### Linear transformation
To eliminate the cell-to-cell technical variability coming from the differences of mRNA capture efficiency within one C1 collection, we next performed linear transformation on the log transformed molecule numbers using a fitted linear model generating from the observed and the expected molecule qualities of ERCC spike-ins in each sample.
In other words, we used ERCC spike-ins to make a linear standard curve for each sample.  
The PCA of gene expression after linear transformation showed that PC2 separated not only the three individuals but also the three C1 replicates of each individual, similar to the results of filtered raw molecule data, suggesting the batch effect from the C1 replicates were yet to be removed.

#### Mixed model for batch effect correction
To directly remove unwanted techncial variation between C1 plates, we modeled between well correlation in gene expression within C1 plates. We assumed that C1 plates share similar techncial variation due to gene expression similarity between cells on the same plate. The results of PCA on the corrected data showed that the samples from different C1 replicates of the same individual largely overlapped, indicating that we have successfully removed large proportion of the unwanted technical variation across C1 replicates. 
...
Poisson plot for each individual
(poisson fit plot for all individuals)
*gene annotation

Top ranked CV similar between individuals due to mean similarity 
(mean and cv venn diagram)

High mean different CV
low mean different CV
...
### Genes with differential cell-to-cell variability across individuals
