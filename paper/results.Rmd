...
Moreover, to estimate how well the scRNA-seq can recapitulate the RNA-seq results from population bulk samples, each individual C1 collection was accompanied by a collection of bulk sample using the same reagents.
...
Accordingly, we didn't observe any obvious technical effects due to sequencing lane or flow cell that confounded the interindividual and interreplicate comparisons (data not shown).
...
To avoid having to choose arbitrary cutoffs, we defined the cutoff for each of these metrics based on the 95th percentile of the distribution for the 18 no-cell libraries (Figure. 1B-E and Sup Figure. 2C-F).
...
After removing the unwanted technical variability from our iPSC scRNA-seq data, we analyzed the cell-to-cell variation by measuring the gene expression noise across single cells and also by identifying genes with differential noise levels across individuals. 

#### Gene expression noise
We first examined endogenous transcriptional variability as a function of RNA abundance across C1 replicates of each individual iPSC lines.
The coefficient of variation (CV) of molecule counts was computed after collision correction but without either standardization or normalization. 
As being reported from the previous studies [@Islam2014; @Brennecke2013], we found similar variation pattern of endogenous genes and ERCC spike-in genes, both of which closely track the expected Poisson distribution as a function of RNA abundance (Sup Figure. 8A). 

To further investigate cell-to-cell variation that was independent of RNA abundance levels, we exploited CV of normalized molecule counts and computed a gene expression noise measure on a gene-by-gene basis of each individual iPSC lines [@Kolodziejczyk2015; @Newman2006]. 
We refer to this individual-specific noise measure as distance-to-the-median (DM, see the Methods section, Sup Figure. 8B). 
Specifically, large DM values correspond to large individual-specific CVs compared to global CVs at the similar RNA abundance levels. 
When looking at the intersection of the top 1,000 individual-specific noisy genes according to their DM values, we found 120 common noisy genes between the three individuals (Figure. 5A), which was smaller than the 891 common highly expressed genes between individuals (Figure. 5B), indicating the presence of differential cell-to-cell variation in gene expression across individuals.  

#### Differential CV genes
We identified differential noisy genes with a permutation-based test under which the labels of iPSC lines were shuffled in each permutation and the median of absolute deviation (MAD) was used to quantify the dissimilarity of noise measures. 
At an alpha-level of $10^{-5}$, 263 genes were found to have statistically significant MAD values (Sup Table. X of gene list, Sup. Figure. 9). 
Enrichment analysis suggested that these 263 genes were enriched for genes involved in protein translation, protein disassembly, and various biosynthetic processes (Sup Table. X).
We did not find any key pluripotency genes to have differential noise measurements across individuals (Sup. Figure. 10), including Nanog and ZFP4 (Figure. 5C and D, respectively), indicating that the pluripotent state was tightly regulated.
More importantly, we observed a number of individual cell-to-cell variation patterns of genes with significant MAD values under the scenarios of differential number of subpopulations, differential proportion of subpopulations, and differential spread or variance (Figure. 5E-F). 
For instance, NLRP2 in individual 19101 differs from the other two individuals in the number of cell populations and the spread of the expression patterns, while the other two cell lines differed in the proportion of subpopulations.
Therefore, our method not only identified differential noisy genes across individuals, but also provided the information regarding the distribution of gene expression across single cells.  
