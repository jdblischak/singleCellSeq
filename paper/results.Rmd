...
To investigate cell-to-cell variation independent of RNA abundance levels, we exploited coefficients of variation of molecule counts and computed a noise measure on a gene-by-gene basis of individual cell lines [@Kolodziejczyk2015, @Newman2006]. We refer to this individual-specific noise measure as distance-to-the-median (DM, see the Methods section, supplemental figure X2). Large DM values correspond to large individual-specific CVs compared to global CVs that are at similar RNA abundance levels. Figure 6b shows the intersection of the top 1,000 individual-specific noisy genes. We found 120 common noisy genes between the individuals (Figure 6b, Supplemental Figure), which is smaller than the 891 common highly expressed genes between individuals (Figure 6a).

We identify differential noisy genes with a permutation-based test under which labels of cell lines were shuffled in each permutation and median of absolute deviation (MAD) was used to quantify the dissimilarity of noise measures. XX genes were found to have statistically significant MAD values at an alpha-level of XXX (supplemental table X of gene list). Figure 6c-f display individual cell-to-cell variation patterns of genes with significant to non-significant MAD values. We observed that significant MAD value can occur under the scenarios of differential number of subpopulations, differential proportion of subpopulations, and differential spread or variance. For instance, NLRP2 in NA19101 differs from the other two cell lines in the number of cell populations and the spread of the expression patterns, while the other two cell lines differ in the proportion of subpopulations. Pathway analysis at FDR < .01 revealed xxx genes (supplemental figure/table). 
