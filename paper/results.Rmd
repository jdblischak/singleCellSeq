...
Surprisingly, while ERCC gene expression level from samples from the same batch are more correlated than data from samples across batches (Kruskal-Wallis test, Chi-squared *P* < 0.001), we found that the proportion of variance explained by the individual was significantly larger than the variance due to C1 batch (median: 9% vs. 5%, Chi-squared test; *P* < 0.001, Figure `r fig_supp_variance`B).
