...
<<<<<<< HEAD
Once we removed the unwanted technical variability from scRNA-seq data as described above, we can focus on analyzing cell-to-cell variation of gene expression at the transcriptional level.
We first investigated the biological gene expression variability, or gene expression noise, across single cells of each individual, and then we further identified the genes that present differential expression noise across the three individuals.

#### Gene expression noise at transcriptional level
By examining endogenous transcriptional variability in iPSCs as a function of RNA abundance across C1 replicates, we found that coefficient of variation (CV) of molecule counts among endogenous genes and ERCC spike-in genes suggested similar expression variability patterns that track the expected Poisson distribution (Sup Figure. 8A), which are consistent with previous studies in mouse ESCs (embryonic stem cells) [@Islam2014; @Brennecke2013].
Due to the observation that transcriptional variability was correlated with mean gene RNA abundance level, we adjusted our variability measure (CV) by mean gene abundance level (see the Methods section, Sup Figure. 8B), a similar method used to investigate gene expression noise in mouse ESCs [@Kolodziejczyk2015].
...
While including both expressed and non-expressed cells for measuring of gene expression noise in each individual, we found that 120 genes were ranked among top 1,000 noisy genes (Figure. 5A), while 891 genes were ranked among top 1,000 highly expressed genes (Figure. 5B), implying the presence of differential cell-to-cell variation in gene expression across individuals.  
We also investigated gene expression noise of the expressed cells - cells that were quantified by one or more molecule counts in each gene, using the same noise measure. 
We found that CV of genes with less than 50% drop-outs decreases as a function of gene abundance, whereas CV of genes with drop-out rates more than 50% were found to increase as a function of gene abundance (Sup. Figure X). 
Interestingly, when all cells were included in the analysis, CV decreases as a function of abundance across drop-out rates, suggesting that gene expression noise of the expressed cells depends on the drop-out rate as well as mean gene abundance.
Similar to expression noise of all cells, only a small number of 89 genes were consistently ranked among the top 1,000 most variable genes between the three individuals, indicating a similar degree of differential variability between the expressed cells and the set of cells that include drop-outs.
However, we found no correlation between the CVs of these two sets of cells (supp. figure X). 
...
While all cells were included, our permutation-based test indicated that 263 genes had statistically significant differences in gene expression noise between individuals, at an alpha-level of $10^{-5}$ (Sup Table. X of gene list, Sup. Figure. 9). 
...
We did not find any key pluripotency genes to have differential noise measurements across individuals (Sup. Figure. 10), including Nanog and ZFP42 (Figure. 5C and D, respectively), indicating that the pluripotent state was tightly regulated.
Furthermore, transcriptional noise variation exhibited a few interesting patterns across individuals, such as proportion and expression variability of the expressed cells, and proportion of drop-outs (non-expressed cells) (Figure. 5E-F).
For instance, NLRP2 in individual 19101 exhibited smaller number of expressed cells compared to the other two individuals, and a more narrow range of expression among the expressed cells compared to individual 19239. 
On the other hand, FAT3 exhibited a similar range of variation in the expressed cells between the three individuals, but the number of expressed cells was different between the individuals. 
While only the expressed cells were included, only 25% of the top 100 deferentially noisy genes in the expressed cells were among the top 100 differential noisy genes in all cells. 
Additionally, the 25 genes that were found to be different in both expression noise and expression variability of the expressed cells were enriched for regulation of biosynthetic process and cellular metabolic process. (Maybe include aa figure that shows the individual density distributions for some of these genes..)
=======
After removing unwanted technical variability from scRNA-seq data as described above, we next focus on analyzing cell-to-cell variation of gene expression at the transcriptional level.
We first investigated the biological gene expression variability, or gene expression noise, across single cells of each individual, and then we identified genes that present differential expression noise across the three individuals.


#### Gene expression noise at transcriptional level
Examining endogenous transcriptional variability in iPSCs as a function of RNA abundance across C1 replicates, we found that coefficient of variation (CV) of molecule counts among endogenous genes and ERCC spike-in genes suggested similar expression variability patterns that track the expected Poisson distribution (Sup Figure. 8A), which are consistent with previous studies of mouse ESCs (embryonic stem cells) [@Islam2014; @Brennecke2013]. Due to the observation that transcriptional variability was correlated with mean gene abundance, we adjusted our gene variability measure (CV) by gene abundance averaged across single cells (see the Methods section, Sup Figure. 8B), a similar method used to investigate gene expression noise in mouse ESCs [@Kolodziejczyk2015]. While including all single cells passed quality filtering, we found that 120 genes were ranked among the top 1,000 noisy genes in all three indivdiuals (Figure. 5A), while 891 gnees were ranked among the top 1,000 highly expressed genes (Figure. 5B), indicating the presence of differential cell-to-cell variation in gene expression across individuals. 

We next focused on gene expression noise of the expressed cells - cells that were detected by one or more molecule counts in each gene, using the same noise measure as before. The proportion of undetected cells varied between genes; the median ranged between 21 to 30 percent in the three indivduals. Expectedly, gene abudance and variance of the expressed cells were associated with percent of undetected cells (supp). The variabilty measure coefficient of variation was not associated with percent of undetected cells (Spearman's correlation = .04) across the three individuals (supp), supporting that CV is not confounded with the dynamic range of gene abundance. In addition, significant individual difference in proportion of undetected genes were found in more than 10 percent of the genes (1,958 genes).  After excluding the undetected cells on a per gene basis, we also found the three individuals shared 887 highly expressed genes and 95 highly noisy genes, similar to the results described earlier while including all quality single cells. 


#### Differential gene expression noise across individuals 
Differential variation analysis included only the expressed cells to compute individual CVs adjusted for gene abundance. Median of absolute deviation (MAD) was used to to quantify the degree of dissimilarity between individual noise measures. Our permutation-based test indicated that 680 genes had statistical signficance difference in expression noise at the transcriptional level, at an alpha level of $10^{-5}$ (Sup Table. X of gene list, Sup. Figure. 9). Enrichment analysis suggested that these genes were enriched for genes involved in protein translation, protein disassembly, and various biosynthetic processes (Sup Table. X). We found two highly expressed pluripotency genes *DNMT3B* and *NR6A1* among the differential noisy genes (Figure. 5E and F, respectively), both of which were detected in all single cells (Sup. Figure. 10). However, majority of the pluripotency genes were not detected in all the cells and exhibited no differential noisy papttern, including key pluripotency genes *Nanog* and *ZFP42* (Figure. 5C and D, respectively), indicating that the pluripotent state was tightly regulated. Finally, despite the association between gene abundance and proportion of undetected genes, we found no associations between the indvidual differences, and similarly for individual differences in expression noise and proportion of undetected genes. 

>>>>>>> 194206f7c71bc7d55b6c312a2001082f050e2eca
