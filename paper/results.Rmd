...
In turn, we describe data as originating from different replicates when we refer to all samples from a given C1 collection, and from different individuals when we refer to data from all replicates of a given genetically distinct iPSC line.
...
Given the low abundance of mRNA content and the high vulnerability of single cells and the low amount of starting material, scRNA-seq can be highly affected by various technical artifacts arising from cell isolation, mRNA capture efficiency, library preparation, and sequencing.
...
We found that, across samples, sequencing reads from the second replicate of individual NA19098 include unusually high ERCC content compared to all other samples and replicates (Figure `r fig_supp_qc`A and `r fig_supp_qc`B). 
...
We proceeded by focusing on the assumption that data from each sample correspond to data from a single cell. 
Before the processing of the single cell samples, we performed visual inspection on the C1 microfluidic plates. 
Based on that visual inspection, we have flagged sequencing data from 21 samples that did not contain any cell, and 54 samples that contained more than one cell (across all batches). 
...
We chose data-driven inclusion cutoffs for each metric, based on the 95th percentile of the distribution for the 21 libraries that were amplified from samples that did not include a cell based on visual inspection (Figure `r fig_supp_qc`C-F). 
...
We found that while gene-specific read and molecule counts are exceptionally highly correlated when we considered the ERCC spike-in data (r = 0.99; Figure `r fig_main_qc`F), these counts are somewhat less correlated when data from the endogenous genes are considered (r = 0.92).
...
Beyond quality control of the samples, we also investigated the effect of sequencing depth and the number of single cells collected.
To this end, we conducted a systematic subsampling to assess the correlation of the single cell gene expression estimates to the bulk samples, the number of genes detected, and the correlation of the cell-to-cell gene expression level variance estimates in the subsampled single cells to the full single cell data set (Figure `r fig_main_subsample`).
We observed quickly diminishing improvement in all three of these metrics with increasing sequencing depth and number of cells collected, especially for the highly expressed genes.
For example, a per cell sequencing depth of 1.5 million reads (which corresponds to ~50,000 molecules) for 75 single cells is sufficient to quantify even the lower 50% of expressed genes.
At this level of subsampling for individual NA19239,
the sum of the single cell gene expression levels correlates with the bulk sample gene expression levels with a mean Pearson coefficient of `r round(d_filter$mean_bulk, digits = 2)` (Figure `r fig_main_subsample`A),
a mean of `r d_filter$mean_detected ` genes out of `r d_filter$used_ensg` expressed genes are detected (Figure `r fig_main_subsample`B),
and the cell-to-cell gene expression level variance estimates of the subsampled single cells correlates with the variance estimates from the full data set with a mean Pearson coefficient of `r round(d_filter$mean_var, digits = 2)` (Figure `r fig_main_subsample`C).
...
However, this conjecture can only be correct if all RNA molecules in single cells are being amplified (‘captured’) and sequenced, or if the capturing rate and sequencing of RNA molecules is highly similar (practically identical) across single cells.

Our study design allows us to carefully examine this notion. 
To do so, we considered the correlation between sample concentrations (measured at the library stage) and total molecule-counts per sample, summed over all detected endogenous genes (between 30,408 to 113,832 molecules per cell; Figure `r fig_main_batch`A).
We only included data from individuals NA19101 and NA19239 in this analysis, because sample concentration measurements from individual NA19098 were not available.
Consistent with previous reports, we observed a positive correlation between total molecule-counts in single cells and the measured sample concentration (Figure `r fig_main_batch`C). 
...
As a consequence, it would be difficult, for example, to correctly infer the relative size of single cells based on the observed number of molecules.

We investigated this property further by inferring the cell cycle phase of single cells based on their gene expression profiles using molecule counts [@Macosko2015; @Whitfield2002].
...
It is expected that cells that are inferred to be in G2 and G2/M phases (larger cells), would have higher total molecule-counts [@Vallejos2015], which is indeed our general observation (Figure `r fig_supp_cellcycle`A and `r fig_supp_cellcycle`B). 
However, variation from this pattern can clearly be seen across single cell collections from different individuals, as well as across different C1 replicates from the same individual (Figure `r fig_supp_cellcycle`). 
...
Indeed, even with 50M sequencing reads (an order of magnitude more than the number of reads sequence in typical single cell experiments), while the number of detected endogenous genes plateaus (Figure `r fig_supp_subsample`A and `r fig_supp_subsample`B), the number of newly identified molecules is still increasing (Figure `r fig_supp_subsample`D and `r fig_supp_subsample`F).
...
Specifically, we found that the proportion of variance due to individual was larger (median: 8%) than variance due to C1 batch (median: 4%, Chi-squared test; *P* < 0.001, Figure `r fig_supp_variance`A; see Methods for details of the variance component analysis). 
...
If only technical variability affects ERCC counts, we expect the magnitude of variance in the spike-in expression profiles to be consistent across batches and individuals, in contrast to the pattern observed for endogenous molecule counts. 
In practice, however, total ERCC molecule-counts were significantly different across C1 batches (F-test; *P* < 0.001), yet not across individuals (LRT; *P* = 0.08;  Figure `r fig_main_batch`B). 
In addition, while we observed a positive relationship between total ERCC molecule-counts and total endogenous molecule-counts per sample, this correlation pattern differed across C1 batches as well (F-test; *P* < 0.001; Figure `r fig_main_batch`D).
...
Surprisingly, when we considered ERCC gene expression levels, we found that the proportion of variance explained by the individual was significantly larger than the variance due to C1 batch (median: 9% vs. 5%, Chi-squared test; *P* < 0.001, Figure `r fig_supp_variance`B).
...
To do so, we focused on the read-to-molecule conversion rates, namely the rates at which we convert sequencing read to molecule counts based on the UMI sequences.
...
Our analysis indicated that molecule counts need to be standardized and that there is a considerable batch effect in the single cell gene expression data.
We thus sought an approach that will take the batch effect into account and allow us to study biological properties of the single-cell molecule count based estimates of gene expression levels, albeit in a small sample of just three individuals.
...
We used the framework of a Poisson generalized linear model to normalize the endogenous molecule log~2~ cpm values by the observed molecule counts of ERCC spike-ins across samples.
...
To account for heteroscedasticity, we adjusted the CV values by the average gene-specific expression level across cells of the same individual.
