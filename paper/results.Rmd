...
For example, a sequencing depth of 1.5 million reads (which corresponds to ~50,000 molecules) for 75 single cells is sufficient for measuring even the lower 50% of expressed genes;
it correlates with the bulk sample gene expression levels with a mean Pearson coefficient of `r round(d_filter$mean_bulk, digits = 2)` (Figure `r fig_main_subsample`A),
detects a mean of `r d_filter$mean_detected ` genes out of `r d_filter$used_ensg` expressed genes (Figure `r fig_main_subsample`B),
and correlates with the cell-to-cell gene expression level variance estimates from the full data set with a mean Pearson coefficient of `r round(d_filter$mean_var, digits = 2)` (Figure `r fig_main_subsample`C).
...
Surprisingly, when we considered ERCC gene expression levels, we found that the proportion of variance explained by the individual effect was significantly larger than the variance due to C1 batch (median: 9% vs. 5%, Chi-squared test; *P* < 0.001, Figure `r fig_supp_variance`B).
