...
We collected single cell RNA-seq (scRNA-seq) data from three YRI iPSC lines using the Fluidigm C1 microfluidic system followed by sequencing (see Methods). 
...
We found that, across samples, sequencing reads from the second replicate of individual NA19098 include unusually high ERCC content compared to all other samples and replicates (Sup Figure. `r fig_supp_qc`A and `r fig_supp_qc`B). 
...
We proceeded by focusing on the assumption that data from each sample correspond to data from an individual cell. 
...
We chose data-driven inclusion cutoffs for each metric, based on the 95th percentile of the distribution for the 21 libraries that were amplified from samples that did not include a cell based on visual inspection (Sup Figure. `r fig_supp_qc`C-F). 
...
We found that while gene-specific read and molecule counts are exceptionally highly correlated when we considered the ERCC spike-in data (r = 0.99; Figure. `r fig_main_qc`G), these counts are somewhat less correlated when data from the endogenous genes are considered (r = 0.92; Figure. `r fig_main_qc`F).
Moreover, the gene-specific read and molecule counts correlation is noticeably lower for genes that are expressed at lower levels (TODO: figure or supplemental figure that shows this).
...
Beyond quality control of the samples, we also estimated the number of molecules required for representing the gene expression level patterns of the bulk samples.
To this end, we conducted a systematic subsampling at the levels of sequencing depth (including both read and molecules) and cell numbers, using the high quality samples (Figure. `r fig_main_subsample`).
We found that sequencing depth of 2 million reads from each cell, which corresponds to ~100,000 molecule counts, with 75 single cells from each individual, is sufficient to capture the majority of genes expressed (~90 %, Figure. `r fig_main_subsample`A), as well as to accurately reflect mean expression levels in the whole cell culture (r > 0.93, Figure. `r fig_main_subsample`B). 
The variation in gene expression levels across individual cells is also estimated well with roughly 75 single cells (r > 0.95, Figure. `r fig_main_subsample`C).
...
However, this conjecture can only be correct if all RNA molecules in individual cells are being amplified (‘captured’) and sequenced, or if the capturing rate and sequencing of RNA molecules is highly similar (practically identical) across individual cells.
...
Specifically, we found that the proportion of variance due to individual was larger (median: 8%) than variance due to C1 batch (median: 4%, Chi-squared test; *P* < 0.001, Sup. Figure. `r fig_supp_variance`A; see Methods for details of the variance component analysis). 
...
Surprisingly, when we considered ERCC gene expression levels, we found that the proportion of variance explained by the individual effect was significantly larger than the variance due to C1 batch (median: 9% vs. 5%, Chi-squared test; *P* < 0.001, Sup. Figure. `r fig_supp_variance`B).
...
We used the framework of a Poisson generalized linear model to normalize the endogenous molecule log~2~ cpm values by the observed molecule counts of ERCC spike-ins across samples (see Methods).
...
Once we removed the unwanted technical variability, we focused on analyzing biological variation in gene expression between individual cells. 
...
Across all genes, the median gene-specific dropout ranged between 21 to 30 percent of cells.
We found significant differences (TODO: specify test; *P* < 0.01) in gene-specific dropout rates between individuals in more than 10% (1958 of `r format(sum(grepl("ENSG", rownames(molecules_filter))), big.mark = ",")`) of expressed endogenous genes.
...
We found similar results when we considered data from all single cells, regardless of whether the gene was detected as expressed (TODO: supplementary figure that shows this).
...
Based on permutations (see Methods, Sup Figure. `r fig_supp_permutation`), we classified the estimates of regulatory noise of 330 genes as significantly different across individuals (empirical permutation *P* < $10^{-5}$; see figure. `r fig_main_noisygene ` for examples; Sup Table. `r table_noisygene` for gene list). 
These 330 genes are enriched for genes involved in protein translation, protein disassembly, and various biosynthetic processes (Sup Table. `r table_GO`). 
