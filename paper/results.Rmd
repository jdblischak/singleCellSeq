...
Yet, we also observed that the relationship between total single cell molecule-counts and concentration, is significantly different across individuals (range of correlation is 0.28 to 0.82; F-test; p < 0.001), as well as across C1 collection replicates from the same individual (range of correlation is -0.04 to 0.82; Likelihood ratio test; p < 0.001). 
...
Using principal component analysis (PCA), we found – as expected - that the major source of variation in data from single cells is the individual origin of the sample (Hotelling’s T-squared test p < 0.001; Figure. 4A). 
Yet, variation due to technical batch is also substantial, with data from single call samples of the same batch more correlated than across batches  (p < .001; Sup Figure. X).
...
We observed a clear difference across the three individuals (F-test; p < 0.001) and also across C1 replicates of the same individuals (Likelihood ratio test, p < 0.001, Sup. Figure. 4C), suggesting that some unidentified factors inherent from the individual difference to the distribution of gene expression in the single cells is consistently affecting the molecule counts.
