...
We observed that the proportion of variance due to individual was larger (median: 20%) than variance due to C1 batch (median: 5%, Chi-squared test; *p*-value < 0.001). 
Yet, variation due to C1 batch is also substantial - data from single cell samples within a batch is more correlated than that from across batches (Kruskal-Wallis test, Chi-squared; *p* < 0.001).
...
To account for the batch effect, we modeled between sample correlation in gene expression within C1 replicate (see Methods).
Our algorithm mimics a method that was initially develops for adjusting within-replicate correlation in microarray data [@Smyth2005].
Essentially, we assumed that C1 replicate share similar technical variation due to gene expression similarity between samples on the same plate. 
...
Thus, we measured the expression noise and gene abundance with or without the single cell samples where a given gene was not detected/expressed (all cells in Figure. 5A and expressed cells in Figure. 5B)[@Shalek2013].
