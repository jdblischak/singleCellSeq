...
Based on permutations (Figure `r fig_supp_permutation`), we classified the estimates of regulatory noise of 330 genes as significantly different across individuals (empirical permutation *P* < 10^-5^; Figure `r fig_supp_noisygene` for examples; Table `r table_noisygene` for gene list).
...
Interestingly, among the genes whose regulatory noise estimates differ between individuals, we found two highly expressed pluripotency genes, *DNMT3B* and *NR6A1* (Figure `r fig_supp_noisygene`; Figure `r fig_supp_plurigene`).
