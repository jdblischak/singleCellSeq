...
In other words, the potential sources of the batch effects are yet to be charaterized, which motivates us to look into it from the following aspects to better distinguish technical variability from biological variability.
...
Even though cells in G2 and G2/M phases have higher total molecule/read counts than cells in G1 and S phase in a few but not all C1 replicates, the differences are not significant due to the large variances, suggesting that the techincal variability is not trivial.
...
After removing the unwanted technical variability from the biological variablity that we are interested in, we next examined endogeneous transcriptional variability as a function of RNA abundance across C1 replicates of individual cell lines.
Coefficient of variation of molecule counts was computed after collision correction and transformations. 
We found similar variation pattern of endogeneous genes and ERCC spike-in genes, and both closely track the expected Poisson distribution as a function of RNA abundance (Sup Figure. 8A). 

To investigate cell-to-cell variation independent of RNA abundance levels, we exploited coefficients of variation of molecule counts and computed a noise measure on a gene-by-gene basis of individual cell lines [@Kolodziejczyk2015, @Newman2006]. 
We refer to this individual-specific noise measure as distance-to-the-median (DM, see the Methods section, Sup Figure. 8B ). 
Specifically, large DM values correspond to large individual-specific CVs compared to global CVs that are at the similar RNA abundance levels. 
When looking at the intersection of the top 1,000 individual-specific noisy genes, we found 120 common noisy genes between the individuals (Figure. 6A), which is smaller than the 891 common highly expressed genes between individuals (Figure. 6B).

We identify differential noisy genes with a permutation-based test under which labels of cell lines were shuffled in each permutation and median of absolute deviation (MAD) was used to quantify the dissimilarity of noise measures. 
263 genes were found to have statistically significant MAD values at an alpha-level of $10^{-5}$ (Sup Table. X of gene list, Sup. Figure. 9). 
Figure 6c-f display individual cell-to-cell variation patterns of genes with significant MAD values. 
We observed significant MAD values under the scenarios of differential number of subpopulations, differential proportion of subpopulations, and differential spread or variance. 
For instance, NLRP2 in NA19101 differs from the other two cell lines in the number of cell populations and the spread of the expression patterns, while the other two cell lines differ in the proportion of subpopulations. 
Enrichment analysis suggests that these 263 genes were enriched for housekeeping genes and encoding ribosomal proteins (Sup Table. X). 
