...
Consistent with previous reportes, we observed a correlation between total molecule-counts in single cells and the measured sample concentration (Sup. Figure. 4B). 
...
Using principal component analysis (PCA) for visualization, we observed – as expected - that the major source of variation in data from single cells is the individual origin of the sample (Figure. 4A). 
Further support was found when we assessed per-gene expression profiles (See Method).
...
Yet, variation due to C1 batch is also substantial - data from single call samples within a batch is more correlated than that from across batches (Kruskal-Wallis test, Chi-squared p < 0.001, Sup. Figure. 7).
...
However, in practice, total ERCC molecule-counts were significantly different across C1 replicates (F-test, p-value < 0.001), yet not across individuals (LRT, p-value = 0.078). In addition, while we observed a positive relationship between total ERCC molecule-counts and total endogenous molecule-counts per sample, this correlation pattern differed across C1 replicates as well (F-test p-value < 0.001; Sup. Figure. 4).
Furthermore, while assessing the ERCC expression profile per sample, we observed.......
Our results therefore indicate that variance in ERCC spike-in is not consistent 
...
To account for the UMI collision probability of genes expressed at different level, we first corrected the raw molecule number of each endogenous gene by using the Poisson approximation (See Methods)[@Fu2011].
Due to the stochasticity of the UMI labeling process, not all molecules will be tagged with an unique UMI and sequenced.
By combining the detected molecule numbers of a given gene and the total 1024 different UMIs, we can simultaneously count the actual number of molecules that are in the cell using the probability of a specific UMI-tagged molecule being detected at least once. 
As a result, we are essentially correcting for the fact that we did not observe all the molecules of any given gene originally in the cell. 
This step also filtered the genes with molecule counts larger than 1024, which in our case were all mitochondrial genes.

We next incorporated a standardization step by computing log transformed counts-per-million (cpm) of endogenous gene molecule-counts.
By doing this, we remove the effect of different sequencing depth, as well as the difference in total molecule-counts due to cell-cell variation. 
We then checked the effect of standardization by visualization of PCA and found that, after the removal of the variance caused by total molecule-counts, the difference between C1 replicates, or the batch effect, became dominant (*p* < Y across replicates from the same individual, Figure. 4A and 4B).

The variation of ERCC spike-in controls across samples within a C1 collection should be affected only by technical variability.
In other words, the differences of mRNA capture efficiency, defined as the proportion of molecule counts of ERCC spike-in being observed, can be used to account for the cell-to-cell technical variability arising from the differences of mRNA capture efficiency across samples within the C1 collection. 
We next performed Poisson GLM (generalized linear model) normalization using the observed and the expected raw molecule counts of ERCC spike-ins in each sample (see Methods).
...
We then modeled between well correlation in gene expression within C1 replicate (see Methods) to remove unwanted technical variation across C1 replicates.
