...
We found that, across samples, sequencing reads from practically all samples of the second replicate of individual NA19098 include unusually high ERCC content compared to all other samples and replicates (Figure `r fig_supp_qc`A and `r fig_supp_qc`B).
...
After the cell sorting was complete, but before the processing of the samples, we performed visual inspection on the C1 microfluidic plates.
...
For example, a per cell sequencing depth of 1.5 million reads (which corresponds to ~50,000 molecules) from each of 75 single cells is sufficient for effectively quantifying even the lower 50% of expressed genes.
At this level of subsampling for individual NA19239, we are able to detect a mean of `r d_filter$mean_detected ` genes out of `r d_filter$used_ensg` expressed in the bulk samples (the bottom 50%; Figure `r fig_main_subsample`B); the estimated single cell expression levels of these genes correlate with the bulk sample gene expression levels with a mean Pearson coefficient of  `r round(d_filter$mean_bulk, digits = 2)` (Figure `r fig_main_subsample`A), and the estimated cell-to-cell variation in gene expression levels based on 75 samples is correlated with the variation estimated from the full data set with a mean Pearson coefficient of `r round(d_filter$mean_var, digits = 2)` (Figure `r fig_main_subsample`C).
...
However, this conjecture can only be correct if all RNA molecules in single cells are being amplified (‘captured’) and sequenced, or if the capture rate and sequencing of RNA molecules is highly similar (practically identical) across single cells.
...
However, we also observed that the relationship between total single cell molecule-counts and concentration is significantly different across individuals (likelihood ratio test (LRT); *P* < 0.001), as well as across C1 replicates from the same individual (F-test; *P* < 0.01, See Method).
...
While a trend exists, there is significant variation in this trend, both within and across C1 replicates.
...
We investigated the relationship between total molecule-count and concentration further by inferring the cell cycle phase of single cells based on their gene expression profiles using molecule counts [@Macosko2015; @Whitfield2002].
...
In contrast to the common intuition, we observed a significant batch effect in the read-to-molecule conversion efficiency of both ERCC (F-test; *P* < 0.05) and endogenous genes (F-test; *P* < 0.001) across C1 replicates from the same individual.
...
Our analysis indicated that molecule counts need to be standardized and that there is a considerable batch effect in the single cell gene expression data collected from the C1 platform.
We thus sought an approach that will account for the batch effect and allow us to study biological properties of the single-cell molecule count based estimates of gene expression levels, albeit in a small sample of just three individuals.
...
Wse assume that samples within each C1 replicate share a component of technical variation, which is independent of biological variation across individuals. We fit a linear mixed model for each gene, which includes a fixed effect for individual and a random effect for batch.
...
We thus estimated mean expression levels and biological noise (using adjusted CV) for each gene, by either including (Figure `r fig_main_noise`A) or excluding (Figure `r fig_main_noise`B) samples in which the gene was not detected/expressed.
