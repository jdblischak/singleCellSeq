...
After filtering for the quality of single cell samples and the expression level of genes, we applied 
Due to the stochasticity of the sampling process, not all molecules will be tagged with a UMI and sequenced. We correct for this “collision probability” following the method applied in 

#### Standardization (cpm)
we first checked if the sequecning depth played a role in total molecule counts per sample and found that the total read counts highly correlated with not only the total molecule counts but also the PC1 of overall gene expression.
...
To eliminate the technical variability coming from mRNA capture efficiency between samples within one C1 collection, we next performed linear transforamtion on the log transformed molecule numbers using a fitted linear model generating from the observed and the expected molecule qualities of ERCC spike-ins in each sample. 
The PCA of gene expression after the linear transformation 

#### Mixed effect model to remove batch effect
