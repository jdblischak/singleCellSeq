...
Our study design, which provides the information of molecule counts, allows us to carefully examine these properties.
...
More importantly, we also found that the cell-cell variance from the same cutoffs can accurately represent all the collected single cell samples (r > 0.95, Figure. 2C).

Whether or not the molecule count can be used as an absolute estimate of a given gene in a cell is dependent on one critical factor: the sequencing saturation point, at which all the molecules are sequenced at least once. 
...
Taken together, we concluded that molecule counts cannot be used as an absolute number of mRNA, and more importantly, standardization of molecule-counts is required. 
...
However, variation in this pattern can clearly be seen across single cell collections from different individuals, as well as across different C1 collection replicates from the same individual (Sup. Figure X). 
...
To explore this further, we next asked if the conversion of read counts to molecule counts is consistent across single cell samples.
...
Unexpectedly, we also observed a batch effect arising from the difference of read-to-molecule conversion efficiency across C1 replicates (Figure. 2B and D). 

In addition to batch effect, we have also observed an "individual effect"- difference across individuals, while investigating the use of molecule-counts in scRNA-seq study.
Surprisingly, the difference of read-to-molecule conversion efficiency across the three individuals was significant for both endogenous genes or ERCC spike-ins (p<0.001, Figure. 2B and D), suggesting that differences in certain unknown factors across these individuals are affecting the molecule counts of both ERCC spike-ins and endogenous genes in single cells.
