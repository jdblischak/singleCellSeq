...
The first step of our analysis approach is to transform the numbers of UMIs (raw molecule numbers) into actual molecule counts in single cell samples.  
...
By combining the raw molecule numbers of the gene and the total 1024 different UMIs, we can simultaneously count the actual number of molecules by computing the probability of a specific UMI-tagged molecule being detected for at least once. 
...
We next incorporated a standardization step by computing log transformed counts-per-million (cpm), the same method that has been used routinely in bulk RNA-seq analyses, of endogenous gene molecule-counts but not ERCC spike-in controls.
...
We checked the effect of standardization by visualization of PCA and found that, after the removal of the variance caused by total molecule-counts, the difference between C1 replicates, or the batch effect, became dominant (*p* < Y across replicates from the same individual, Figure. 4A and 4B).

As discussed previously, the variance of ERCC spike-in controls across samples within a C1 replicate should be affected only by technical variability.
In other words, the differences of mRNA capture efficiency, defined as the proportion of molecule counts of ERCC spike-in being observed, can be used to account for the cell-to-cell technical variability of endogenous gene capture efficiency across samples within C1 replicate.
...
The PCA of gene expression after normalization revealed that PC1 and PC2 separated not only the three individuals (*p* < Y) but also the three C1 replicates of each individual (*p* < Y), suggesting that the batch effect of C1 replicates were yet to be removed (Figure. 4C).
...
Our first goal is to find a proper measurement of the gene expression noise across single cell samples from the same individual.
While examining gene expression noise (using CV) as a function of RNA abundance (mean) across C1 replicates, we found that CV of molecule counts among endogenous genes and ERCC spike-in controls suggested similar expression variability patterns that track the expected Poisson distribution (Sup Figure. 8A), which is consistent with previous studies [@Islam2014; @Brennecke2013]. 
To account for this relationship between CV and mean, we adjusted our CV values by gene abundance averaged across single cells (see the Methods section, Sup Figure. 8B), a similar method used to investigate gene expression noise in mouse embryonic stem cells [@Kolodziejczyk2015]. 

Another important aspect of analyzing scRNA-seq data is the consideration of genes possibly being missed during sequencing (referred to as gene “dropout”) due do low mRNA abundance as well as low mRNA capture efficiency [@Brennecke2014].
Thus, we measured the expression noise with or without the single cell samples where a given gene was not detected/expressed [@Shalek2013]. 
We first examine the presence of differential gene expression noise across individuals by looking at the top 1,000 genes rank by expression noise (CV) and by abundance (mean).
...
To investigate the effects of the dropouts in our analyses, we looked at the proportion of undetected cells, or the dropout rate, of each gene and its association with gene abundance and gene expression noise (Figure. 5C and 5D).
