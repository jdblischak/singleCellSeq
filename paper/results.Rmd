...
We investigated this property further by classifying the cell cycle phase of single cells based on their gene expression profiles using standardized molecule counts [@Macosko2015; @Whitfield2002]. 
...
Using principal component analysis (PCA), we found that single cell samples were separated by individuals, despite that individual 19239 might be dissimilar from the other two individuals (Figure. 4A).
We analyzed this futher by computing the distances between cells based one PC1 and PC2.
Not surprisingly, the separation of different replicates from the same individual was also substantial (p< X; Sup Figure. X).
These results provid the indication that batch effects significantly impact the results of single cell gene expression data.

As described previously, we also explored the technical effcts that affect the total gene molecule-counts of single cell sample.
The analyses of the total gene molecule-counts and its corelation with sample concentration (Sup. Figure. 4B) show that there are significant differences across individuals (p < 0.001) and also across C1 replicates of the same individual (p < 0.001 in individual 19239).
Moreover, our study design allows us to carefully examine this difference across C1 replicates and also individuals by focusing on the molecule counts of ERCC spike-in controls.
In the absence of technical noise, the total ERCC molecule-counts per sample is the same, not affected by the total gene molecule-counts or the percentage of ERCC molecules among all molecules (Sup. Figure. 4C and 4D, repectively).
However, in practice, when looking at the association of total ERCC molecule-counts and total gene molecule-counts, we observed a clear differences across the three individuals (p < 0.001) and also across C1 replicates of the same individuals (p < 0.001 in individual 19098 and 19101; Sup. Figure. 4C). 
Similarly, we also observed significant differences across repilcates from the same individuals (p < 0.05 in all three individuals), and more so across individuals (p < 0.001; Sup. Figure. 4D), suggesting that some unidentified factors inherent from the individual difference to the distribution of gene expression in the single cells is consistently affecting the molecule counts.
