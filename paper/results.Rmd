...
Specifically, we found that the proportion of variance due to individual was larger (median: 8%) than variance due to C1 batch (median: 4%, Chi-squared test; *p*-value < 0.001, Sup. Figure. 4; see Methods for details of the variance component analysis). 
...
Perhaps even more importantly, when we considered ERCC gene expression levels, we found that the proportion of variance explained by the individual effect was significantly larger than the variance due to C1 batch (median: 9% vs. 5%, Chi-squared test; *p*-value < 0.001).
...
Gene dropout is the term used to describe the lack of molecule representation of an expressed gene, presumably due to the relatively low RNA capturing efficiency in single cells [@Brennecke2013].
...
As expected, estimated expression levels and the estimated variation in expression levels across cells, are both associated with gene-specific dropout rates (Sup. Figure. 9A and 9B, respectively). 
However, importantly, adjusted CVs are not associated with gene dropout rates (Spearman's correlation = 0.04; Sup. Figure. 9C), indicating that – at the resolution afforded by our RNA sequencing depth - adjusted CV measurements are not confounded by the dynamic range of single-cell gene expression levels. 

We thus estimated mean expression levels and regulatory noise (using adjusted cv) for each gene, by either including (Figure. 5A) or excluding (Figure. 5B) samples in which the gene was not detected/expressed [@Shalek2013].
...
We found similar results when we considered data from all single cells, regardless of whether the gene was detected as expressed (data not shown).
...
Based on permutation (see Methods), we classified the estimates of regulatory noise of 330 genes as significantly different across individuals (alpha-level of $10^{-5}$; see figure 6 for examples; Sup Table. 2 for gene list, Sup. Figure. 10). 
Differential noisy genes across individuals using the median of absolute deviation (MAD) to quantify the degree of dissimilarity between individual noise measures. 
...
Enrichment analysis suggested that these 330 genes were enriched for genes involved in protein translation, protein disassembly, and various biosynthetic processes (Sup Table. 2). 
Interestingly, among the differential noisy genes whose regulatory noise estimates differ between individualsthat we identified, we found two highly expressed pluripotency genes, *DNMT3B* and *NR6A1*, both of which were detected in all single cells (Sup. Figure. 10).
