...
As described previously, we also explored batch effect of total molecule-counts per sample.
The analyses of the total molecule-counts showed that its relationships with sample concentration (Sup. Figure. X), as well as with the cell cycle pattern (Sup. Figure. Y), are different across individuals and also across C1 replicates of the same individual, providing the indication that technical effects significantly impact the results of single cell gene expression data.
...
In addition, the difference of read-to-molecule conversion efficiency across the three individuals was significant not only in endogenous genes but also in the ERCC spike-ins (p<0.001, Figure. 2B and D), suggesting that differences in certain unknown factors across these individuals are affecting the molecule counts of both ERCC spike-ins and endogenous genes in single cells.

To more rigorously investigate the difference of molecule counts across replicates and also across individuals, we focused on the ERCC spike-in control due to the fact that the molecule count   
