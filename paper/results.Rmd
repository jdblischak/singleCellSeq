...
Based on that visual inspection, we have flagged sequencing data from 21 samples that did not contain any cell, and 54 samples that contained more than one cell (across all replicates). 
...
To this end, we conducted a systematic subsampling at the levels of sequencing depth (including both read and molecule counts) and cell numbers, using the high quality samples (Figure. `r fig_main_subsample`).
...
importantlyThe variation in gene expression levels across individual cells is also estimated well with roughly 75 single cells (r > 0.95, Figure. `r fig_main_subsample`C).
