...
For example, a per cell sequencing depth of 1.5 million reads (which corresponds to ~50,000 molecules) from each of 75 single cells, is sufficient for effectively measuring even the lower 50% of expressed genes.
...
Indeed, both the number of detected endogenous genes (Figure `r fig_supp_subsample`A) and the total number of molecules (Figure `r fig_supp_subsample`C) had still not plateaued at a sequencing depth of 80 million reads per single cell (an order of magnitude more than the number of reads sequenced in typical single cell experiments).
...
Essentially, we assume that samples within each C1 replicate share a component of technical variation, which is independent of biological variation across individuals. We fit a linear mixed model for each gene, which includes a fixed effect for individual and a random effect for batch. 
