...
To assess the accuracy of single-cell RNA-seq, we collected data from bulk sample (cells in bulk) together with each C1 prep from the same population of cells using the same SMARTer master mixes. 
...
### A novel pipeline 
To remove the technical variability arosed from all the factors mentioned above without explicitly modeling the variability of ERCC spike-in molecule counts, we developed a pipeline which is applicable for studying cell-cell variability aross multiple biological conditions with multiple C1 collections.  
