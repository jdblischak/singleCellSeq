...
Ultimately, these parameters should be determined based on the goal of the study. In our case, we focused on recapitulating gene expression levels and inter-individual regulatory differences, estimated from the bulk samples, using the single cell gene expression data. 
...
ERCC spike-in controls have been systematically included in numerous scRNA-seq studies for multiple purposes: to filter for low quality cells, to infer the cellular mRNA content, and to model the technical variability across cells [@Brennecke2013; @Grun2014; @Ding2015; @Vallejos2015]. 
While using the proportion of ERCC spike-in reads as a filter of potential dead or ruptured cells, we removed a small group of samples with higher percentages, yet we also observed a continuous distribution of the samples (Figure. `rfig_main_qc`E), indicating that choosing a cutoff of the proportion of ERCC read is not as obvious as one anticipated. 
Therefore, instead of an arbitrary cutoff, we applied a cutoff according to the proprotion of ERCC read from samples that did not contain any cell.

ERCC spike-ins has been used as the estimation of cellular mRNA contents and also to model technical variability across samples based on the assumption that the proportion of ERCC molecule can serve as an indicator of the amount of total endogenous gene molecules [@Vallejos2015]. 
As described earlier, the differences of total ERCC molecule-counts across individuals as well as across C1 replicates caused by unknown factors have never been reported nor taken into account while modeling the technical variability of scRNA-seq data. 
Moreover, even though ERCC spike-in controls can provide the information required to create a standard baseline measurement of RNA abundance across samples, as well as to determine both the dynamic range and the lower detection limit [@Mortazavi2008], the strong library preparation effects have been observed in bulk RNA-seq data as well [@Risso2014; @SEQC/MAQC-IIIConsortium2014]. 
Furthermore, while ERCC spike-in controls cover a wide range of concentration, the fact that they are relative short in length and have shorter polyA tails compared to mammalian mRNAs brings up the question of whether these differences will affect mRNA capture efficiency. 
...
Our approach to quantifying gene regulatory noise accounts for major contributing factors of cell-to-cell variation in scRNA-seq expression data, including dropout and dynamic range of gene expression. 
Importantly, gene regulatory noise was considered among the detected cells, excluding cells with no detected UMI (range between 21 to 30 percent across genes). 
Our findings that there exists pervasive inter-individual difference in gene regulatory noise beyond gene abundance indicates the potential for single cell expression data to provide additional insight into gene regulatory variation beyond bulk-sample expression data.

To our knowledge, this study is the first that attempts to quantify inter-individual differences in gene regulatory noise using scRNA-seq data. 
While cell-to-cell variation is a popular topic in scRNA-seq experiments, the field has focused on comparisons between homogeneous cell populations or cell types in mammalian cell lines [@Vallejos2016]. 
