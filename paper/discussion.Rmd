...
However, to fully capture the batch effect across C1 plates and to retrieve the maximum amount of biological information, we recommend a balanced study design consisting of multiple individuals within a C1 plate and multiple C1 replicates (for example, see supplement figure). 
In such a scenario, single cells can be assigned to biological samples based on their genotypes, which can be characterized based on the RNA sequencing (see XXX). 
...
In our hands, only 43 out of the 92 ERCC spike-ins passed our expression filter and were used for normalization across samples (Figure. 1G).
...
This sampling bias is also evident across individuals where individual NA19098 have both higher percentage of ERCCs and higher ERCC molecule counts compared to individual NA19239 (Figure. 3C and Sup Figure. 7A-C).
...
Our observation that inter-individual differences in cell-cell variation of gene expression exist independent of gene abundance, to our knowledge, is the first in human cell line, and directly supports the notion that single cell level gene expression offers additional insight to gene regulation beyond bulk-sample gene expression. Similar work in mouse embryonic stem cells (e.g., [@Vallejos2016]) found that relative to cells in G1 phase, cells in G2/M phase were more tightly regulated in genes related to DNA replication regulation, independent of gene abundance levels. Additionally, consistent with previous findings, we found strong correlation between gene dropout rate and cell-cell variation, as well as gene abundance. By accounting cell-cell variation only among cells with detectd transcripts, our finding further supports individual difference in gene regulatory noise beyond randomness in cell sampling. 

Recently, a phenotypic variation study of allele-specific expression using single-cell RNA-seq in human primary fibroblast suggest that at the cell level, there exists mostly monoallelic expression that is stochastic and independent of the parent of origin of the allele [@Borel2015]. Further study of phenotypic variation in cell-cell variation may offer insight into relative robustness of genetic variation -- the change in variation of one or more specific trait that is observed under environmental perturbation. 
