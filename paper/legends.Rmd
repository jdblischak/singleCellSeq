...
(C) The correlation of cell-cell variance in gene expression between a subsampled set of single cells and all collected cells of individual NA19239. 
...
(C) PCA of gene expression using the Possion- normalized molecule numbers. 
(D) PCA of gene expression using the batch-corrected molecule numbers. 
The colors represent the three individuals where NA19098 is in red, NA19101 in green, and NA19239 in blue. 
Different shapes represent different C1 replicates of the individual.

### Figure 5. Cell-cell variation in gene expression. 
(A and B) Coefficients of variation (adjusted CV, see Method) plotted against average molecule counts across all cells in (A) or across only the expressed cells in (B) from all three individuals. 
Each dot represents a gene, whereas the different colors indicate the differences in dropout rate (the proportion of undetected cells).
(C and D) Venn diagrams showing the overlaps of top 1000 genes across individuals based on mean expression level in (C) or based on distance-to-the-median (DM) values in (D) across cells. 

### Figure 6. Differential CV genes across individuals. 
Density plots showing the distribution of log2 gene expression of the genes across all single cells by individual (upper panel).
Additionally, the proportion of detected cells and the absolute value of adjusted CV were plotted in the middle and the lower panels, respectively. 
The dark grey indicates the proportion of detected cells, whereas the light grey indicates the proportion of undetected cells in the middle panel. 
The gene name, the MAD (median of absolute deviation) value, and the *P* values across individuals of each gene were indicated. 
The colors represent the three individuals where NA19098 is in red, NA19101 in green, and NA19239 in blue.
...
### Sup. Figure 8. Cell-cell variation of pluripotency genes.  
...
### Sup. Figure 11. Coefficients of variation (CV) and distance-to-the-median (DM). 
(A-C) Coefficients of variation plotted against average molecule counts across all cells of each individual. 
Grey dots represent endogenous genes, and blue dots indicate ERCC spike-in control genes. 
Red curve depicts the expected coefficients of variation assuming the endogenous genes follow a Poisson distribution. 
Likewise, blue curve depicts the expected CVs of the ERCC spike-in controls. 
Yellow curve predicts the expected CVs assuming standard deviation is three times the ERCC spike-ins. 
(D-F) Distance-to-the-median (DM) values of each gene from all cells are plotted against log10 of the average molecule counts of each individual.
