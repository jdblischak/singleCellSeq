...
### Figure `r fig_supp_variance`. Source of cell-to-cell variance in per-gene expression profile.
Box plots displaying the proportion of per-gene cell-to-cell variance that is due to individual sample of origin, different C1 replicates, and other cell sample differences. Data is based on molecule counts before sample normalization and batch correction. Endogenous genes are shown in (A) and the ERCC spike-in controls in (B).
