...
Violin plots of the number of total molecule-counts in thousands in single cell samples by C1 replicate.
The endogenous genes and the ERCC spike-ins are shown separately in (A) and (B), respectively.
(C) Scatterplot of the sample concentration of cDNA amplicons (ng/ul) and total gene molecule-counts.
(D) Scatterplot of the total ERCC molecule-counts and total gene molecule-counts.
(E and F) Violin plots of the reads to molecule conversion efficiency (total molecule-counts divided by total read-counts per single cells) by C1 replicate.
...
Selected genes are featured and plotted in the upper panel, the distribution of log2 gene expression levels, considering only cells in which the gene is detected as expressed.
...
Each single cell sample were assigned a cell cycle phase (G1/S, S, G2, G2/M, and M/G1) according to its expression data of a subset of genes using molecule counts either before in (A) or after in (B) applying the correction (standardization, normalization, and batch correction, see Method).
Each point represents the mean +/- sem of the total gene molecule-counts per sample before the correction.
(C and D) Same as as in (A) and (B) but for the mean +/- sem of the total read-counts.
(E and F) The total cell numbers of each cell cycle phase are shown before or after correction in (E) and (F), respectively.
Different colors represent different C1 replicates of the same individual.

### Sup. Figure 4. Subsample of high coverage samples.
Four single cells from the lymphoblastiod cell line of NA19239 were each sequenced separately on one lane of a Hi-seq 2000.
This resulted in sequencing depths greater than 50 million per single cell.
\(A\) The number of endogenous genes detected with at least 1 molecule (left panel) or 10 reads (right panel).
(B) Same as in (A) but for the ERCC spike-in controls.
(C) The total number of reads mapping to endogenous genes per single cell as a function of sequencing depth.
(D) The total number of molecules mapping to endogenous genes per single cell as a function of sequencing depth.
(E) The total number of reads mapping to ERCC spike-ins per single cell as a function of sequencing depth.
(F) The total number of molecules mapping to ERCC spike-ins per single cell as a function of sequencing depth.
...
### Sup. Figure 6. The proportion of cells in which the gene is undetected
The proportion of cells in which the gene is undetected and its relationship with log10 mean expression in (A), with log10 variance of expression in (B), and with the CV in (C) of the cells in which the gene is expressed (cells that were quantified by one or more molecule counts of the given gene).
...
### Sup. Figure 10. The proportion of genes detected in single cell samples.
