...
Specifically, for each gene, we computed empirical *P*-values based on a large number of 226,520 permutations. In each permutation, the sample of origin labels were shuffled between cells. Because the number of permutations in our analysis is smaller than the maximum possible number of permutations, we computed the empirical *P*-values as $\frac{b + 1}{m + 1}$, where *b* is the number of permuted MAD values that is greater than the observed MAD value, and *m* is the number of permutations. This computation of adding 1 to *b* avoids the empirical *P* value of zero [@Phipson2010]. Finally, we identified significant genes at false discovery rate of 1%.
