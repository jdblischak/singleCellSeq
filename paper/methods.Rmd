...
Note that the goal here is different from that of the previous section, where we simply tested for the existence of individual and batch effects by rejecting the null hypothesis of no such effects.
In contrast, here we fit a linear mixed model on a per-gene expression (log scale), building on the model described in the previous section, with an additional random component of individual effect.
