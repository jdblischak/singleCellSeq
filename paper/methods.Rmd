...
$$ MAD_{g} = Median_{i= 1,2,3} \left| adj-CV_{g,i} - Median{(adj-CV)}_{g} \right|. $$
...
In each permutation, the cell line labels were shuffled between cells. 
The permutation-based *P*-value was computed as

$$ \frac{1}{B} \sum_{B=1}^{`r format(sum(grepl("ENSG", rownames(molecules_filter))), big.mark = ",")`} \mathbb{I} \left( MAD_{g} > MAD_{g,B} \right),$$
...
We set the statistical significance threshold at 1/`r format(sum(grepl("ENSG", rownames(molecules_filter))), big.mark = ",")`, thus a gene was identified as a differential noise gene when the permutation *P*-value is less than 1/`r format(sum(grepl("ENSG", rownames(molecules_filter))), big.mark = ",")`. 
