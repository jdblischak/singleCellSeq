...
The capture efficiency and TRA-1-60 staining were then inspected using the EVOS FL Cell Imaging System (ThermoFisher)(Sup. Table 1A). 
...
The master mixes, one mix with all the bulk samples and nine mixes corresponding to the three replicates for the three individuals, were sequenced across four flowcells using a design to minimize the introduction of technical batch effects (Sup. Table 1B). 
...
In an initial analysis investigating the percentage of reads mapping to the ERCC spike-in controls, we observed that replicate 2 of individual 19098 was a clear outlier (Sup Figure. 1A and 1B).
...
We chose these criteria based on the distribution of these metrics in the empty wells (95 %, Sup Figure. 1D-F).
Lastly, we observed that some wells classified as containing only one cell clustered with multi-cell wells when plotting the number of gene molecules versus the concentration of the samples (Sup Figure. 2A and 2B) and also the read to molecule conversion efficiency, calculated by total molecule number divided by total read number, of endogenous genes versus that of ERCC (Sup Figure. 2C and 2D).
...
We observed that the levels of ERCC spike-ins strongly correlated with the input quantities (r = 0.9914, Figure. 1G).
...
Individual effect and batch effect between the individual cell samples were evaluated in a series of analyses that examine the potential sources of technical variations on gene expression measurements. These analyses took into consideration that in our study design, sources of variation between individual cell samples naturally fall into a hierarchy of individuals and C1 batches. In these analyses, the variations introduced at the individual-level and at the batch-level were modeled in a nested framework that allows random noises between C1 batches within individuals. Specifically, for each cell sample in individual $i$, replicate $j$ and well $k$, we use $y_{ijk}$ to denote some sample measurement (e.g., total molecule-counts), and $x_{ijk}$ to denote the covariate, if any, that is associated with $y_{ijk}$ (e.g., sample RNA concentration measurement), and fit a linear mixed model with the fixed effect of individual $\alpha_i$ and the random effect of batch $b_{ij}$:

$$y_{ijk} = x_{ijk} + \alpha_{i} + b_{ij} + \epsilon_{ijk}$$

where the random effect $b_{ij}$ of batch follows a normal distribution with mean zero and variance $\sigma^2_{b}$, and $\epsilon_{ij$ describes residual variation in the sample measurement. To test the statistical significance of individual effect (i.e., null hypothesis $\alpha_1 = \alpha_2 = \alpha_3$, we perform likelihood ratio test (LRT) for comparing the above full model and the reduced model that excludes $\alpha_i$. To test if there is a batch effect (i.e., null hypothesis  $\sigma^2_b = 0$), we perform a F-test for comparing the variance that is explained by the above full model and the variance due to the reduced model that excludes $b_{ij}$. 

The nested framework was applied to the testing of individual effect and batch effect between cell samples in the following cases. The data includes samples after quality control and filtering. 

1. Total molecule-count per thousand for the endogenous genes was modeled as a function of the covariate sample RNA concentration, individual effect and batch effect. We excluded individual 19098 from the analysis, because sample concentration was quantified in only one of the two C1 replicates. 
...
To assess the relative contribution of individual variation to technical variation, we analyzed per-gene expression profile and computed variance component estimates of individual effect and batch effect. The goal here is to quantify proportion of cell-cell variance per gene due to individual effect and to batch, which is different from the previous section, which aims to test the statistical significance of individual effect and batch effect. In this section, we fit a linear mixed model on per-gene expression (log scale) building on the model described in the previous section with an additional random component of individual effect. 

The variance parameters of individual effect and batch effect are estimated using a maximum penalized likelihood approach [@Chung2013], which can effectively avoid the common issue of zero variance estimates when sample size is small (we have three individuals and eight batches). We used the blmer function in the R package blme and set the default penalty function as the logarithm of a gamma density (shape parameter = 2 and rate parameter tends to zero). 

The estimated variance components are used to compute the estimated sum of squared (SS) deviations for individual effect and batch effect. The proportion of variance due to each effect is equal to the relative contribution of SS for each effect to total per-gene SS. Finally, we compared the estimated proportions of variance across genes between the individual effect and the batch effect in a non-parametric one-way analysis of variance, also known as Kruskal-Wallis rank sum test.
...
where $\widehat{b}_{g,ij}$ are the LS estimates. The computations in this step are done with the gls.series function of the limma package [@limma].
...
To compare the results before and after applying our normalization method, we used the raw molecule counts (Sup. Figure 3. A, C, and E) and the corrected molecule numbers (Sup. Figure. B, D, and F) as the input data files for the classification.
The raw molecule counts were used to calculate the total gene molecule-counts for both classifications (Sup. Figure 3. A and B).
