...
The master mixes, one mix with all the bulk samples and nine mixes corresponding to the three replicates for the three individuals, were sequenced across four flowcells using a design to minimize the introduction of technical batch effects (`r table_supp_collection`B). 
...
We chose the above criteria based on the distribution of these metrics in the empty wells (95 %, Figure `r fig_supp_qc`D-F). 
...
We therefore established filtering criteria for these misidentified one-cell wells using linear discriminant analysis (LDA).
...
The capture efficiency, defined as the fraction of total input molecules being successfully detected in each high quality cells, had an average of 6.1%.
...
To assess the impact of sequencing depth, we cacluated the following three statistics:

1.  The Pearson correlation of the gene expression estimates from the single cells compared to the bulk samples.
For the single cells, we summed the gene counts across all the samples and then calculed the log~2~ cpm of this pseudo-bulk.
...
3.  The Pearson correlation of the cell-to-cell variance per gene from the subsampled single cells compared to the variance estimates using the full data set.
...
In these analyses, the variation introduced at the individual-level and at the batch-level were modeled in a nested framework that allows random noise between C1 batches within individuals. 
...
## Acknowledgment
We thank Nicholas E. Banovich for the Yoruba iPSC lines; the University of Chicago Genomics Facility for performing Next Generation Sequencing; and members of the Pritchard and Gilad laboratories for valuable discussions during the preparation of this manuscript.
