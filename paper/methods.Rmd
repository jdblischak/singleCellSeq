...
We collected four individual cells (A9E1, B2E2, B4H1, and D2H2) which were then sequenced individually on one sequencing lane (Sup Figure. 6). 
...
We observed that the levels of ERCC spike-ins strongly correlated with the input quantities (r = 0.9914).
...
### Remove technical batch effect due to between-sample gene expression correlatinos 
within C1 replicates
...
To compare the results before and after applying our normalization method, we used the raw molecule counts (Sup. Figure. A, C, and E) and the corrected molecule numbers (Sup. Figure. B, D, and F) as the input data files for the classification.
The raw molecule counts were used to calculate the total gene molecule-counts for both classifications (Sup. Figure. A and B).
