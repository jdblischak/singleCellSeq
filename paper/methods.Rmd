...
We performed multiple quality control analyses to detect and remove low quality cells. In an initial analysis investigating the percentage of reads mapping to the ERCC spike-in controls, we observed that replicate 2 of individual NA19098 was a clear outlier (Sup Figure. `r fig_supp_qc`A and `r fig_supp_qc`B). It appeared that too much ERCC spike-in mix was added to this batch, which violated the assumption that the same amount of ERCC molecules was added to each cell. Thus, we removed this batch from all of our analyses.
...
We chose the above cutoffs based on the distribution of these metrics in the empty wells (95 %, Sup Figure. `r fig_supp_qc`D-F). In addition, we observed that some wells classified as containing only one cell were clustered with multi-cell wells when 1) plotting the number of gene molecules versus the concentration of the samples (Sup Figure. `r fig_supp_lda`A and `r fig_supp_lda`B), and 2) plotting the endogenous versus ERCC gene read to molecule conversion efficiency, calculated by total molecule number divided by total read number (Sup Figure. `r fig_supp_lda`C and `r fig_supp_lda`D). We established filtering criteria for these misidentified one-cell wells using linear discriminant analysis (LDA). Specifically, for each of the two scenarios separately, LDA was performed to classify wells into empty, one-cell, and two-cell using the discriminant function of 1) sample concentration and the number of gene molecules, and 2) endogenous and ERCC gene read to molecule conversion efficiency. Lastly, after filtering, we maintained `r nrow(anno_filter)` high quality single cells
...
We removed all genes with a mean log~2~ cpm less than 2, which did not affect the relative differences in the proportion of genes detected across batches (Sup Figure. `r fig_supp_proportion`).
...
To assess the relative contributions of individual and technical variation, we analyzed per-gene expression profiles and computed variance component estimates for the effects of individual and batch (Sup. Figure. `r fig_supp_variance`).
...
First, squared coefficients of variation (CVs) for each gene were computed for each individual and also across cell lines, using the batch-corrected molecule data.
...
To identify differential DM (TODO: define DM) between individuals, we computed median absolute deviation (MAD) - a robust and distribution-free dissimilarity measure for gene $g$:
...
TODO: We thank… This work was funded by…
