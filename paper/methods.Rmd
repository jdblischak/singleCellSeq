...
The capture efficiency and TRA-1-60 staining were then inspected using the EVOS FL Cell Imaging System (Thermo Fisher) (Table `r table_collection`A).
...
The average yield of samples per C1 plate ranged from 1.26-1.88 ng per microliter (Table `r table_collection`A).
...
The master mixes, one mix with all the bulk samples and nine mixes corresponding to the three replicates for the three individuals, were sequenced across four flowcells using a design aimed to minimize the introduction of technical batch effects (Table `r table_collection`B).
...
In an initial analysis investigating the percentage of reads mapping to the ERCC spike-in controls, we observed that replicate 2 of individual NA19098 was a clear outlier (Figure `r fig_supp_qc`A-B).
...
Specifically, LDA was performed to classify wells into empty, one-cell, and two-cell using the discriminant functions of 1) sample concentration and the number of gene molecules (Figure `r fig_supp_lda`A-B) and 2) endogenous and ERCC gene read to molecule conversion efficiency (Figure `r fig_supp_lda`C-D).
...
Each data point in Figure `r fig_main_subsample` represents the mean +/- the standard error of the mean (SEM) of 10 random subsamples of cells.
...
In these analyses, the variation introduced at both the individual-level and the batch-level was modeled in a nested framework that allows random noise between C1 batches within individuals.
Each gene was analyzed independently.
Specifically, for each cell sample in individual $i$, replicate $j$ and well $k$, we used $y_{ijk}$ to denote some sample measurement (e.g., total molecule-counts), and $x_{ijk}$ to denote the covariate, if any, that is associated with $y_{ijk}$ (e.g., sample RNA concentration measurement), and fit a linear mixed model with the fixed effect of individual $\alpha_i$ and the random effect of batch $b_{ij}$:
...
The goal here was to quantify the proportion of cell-to-cell variance per gene due to individual effect and to batch, which is different from the previous section, which aimed to test the statistical significance of individual effect and batch effect.
...
The proportion of variance due to each effect is equal to the relative contribution of SS for each effect compared to the total per-gene SS.
...
Both endogenous and ERCC spike-in control CV patterns approximately followed an over-dispersed Poisson distribution (Figure `r fig_supp_CV`A-C), which is consistent with previous studies [@Islam2014; @Brennecke2013].
...
After this transformation step, CV no longer had a polynomial relationship with mean gene molecule count (Figure `r fig_supp_CV`D-E).
