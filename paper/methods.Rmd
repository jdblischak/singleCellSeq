...
Single cell loading and capture were performed following the Fluidigm manual "Using C1 to Generate Single-Cell cDNA Libraries for mRNA Sequencing Protocol" (PN 100-7168).
...
Immediately after imaging, reverse transcription and cDNA amplification were performed in the C1 system using the SMARTer PCR cDNA Synthesis kit (Clontech) and the Advantage 2 PCR kit (Clontech) according to the instructions in the Fluidigm user manual with minor changes to incorporate UMI labeling [@Islam2014]. Specifically, the reverse transcription primer and the 1:50,000 Ambion® ERCC Spike-In Mix1 (Life Tech) were added to the lysis buffer, and the template-switching RNA oligos which contain the UMI (5-bp random sequence) were included in the reverse transcription mix [@Islam2011; @Islam2012; @Islam2014]. When the run finished, full-length, amplified, single-cell cDNA libraries were harvested in a total of approximately 13 ul C1 Harvesting Reagent and quantified using DNA High Sensitivity LabChip (Caliper). The average yield of samples per C1 plate ranged from 1.26-1.88 ng per microliter (`r table_supp_collection`A).
...
The master mixes, one mix with all the bulk samples and nine mixes corresponding to the three replicates for the three individuals, were sequenced across four flowcells using a design to minimize the introduction of technical batch effects (`r table_supp_collection`B). Single-end reads of 100 bp were generated along with 8-bp index reads corresponding to the cell-specific barcodes. We didn't observe any obvious technical effects due to sequencing lane or flow cell that confounded the inter-individual and inter-replicate comparisons.

We also collected scRNA-seq data from Lymphoblastoid Cell Lines (LCLs) in a trial run.
We sequenced four single cells, each in their own separate lane.
...
In an initial analysis investigating the percentage of reads mapping to the ERCC spike-in controls, we observed that replicate 2 of individual NA19098 was a clear outlier (Figure `r fig_supp_qc`A and `r fig_supp_qc`B). It appeared that too much ERCC spike-in mix was added to this batch, which violated the assumption that the same amount of ERCC molecules was added to each cell. Thus, we removed this batch from all of our analyses.
...
We chose the above criteria based on the distribution of these metrics in the empty wells (95 %, Figure `r fig_supp_qc`D-F). In addition, we observed that some wells classified as containing only one cell were clustered with multi-cell wells when plotting the number of gene molecules versus the concentration of the samples (Figure `r fig_supp_lda`A and `r fig_supp_lda`B), and also when plotting the read to molecule conversion efficiency, calculated by total molecule number divided by total read number, of endogenous genes versus that of ERCC (Figure `r fig_supp_lda`C and `r fig_supp_lda`D). We therefore established filtering criteria for these misidentified one-cell wells using linear discriminant analysis (LDA).
...
We observed that the levels of ERCC spike-ins strongly correlated with the input quantities (r = 0.9914, Figure `r fig_main_qc`G). The capture efficiency, defined as the fraction of total input molecules being successfully detected in each high quality cells, had an average of 6.1%.

### Cell cycle phase classification

Cell cycle analysis was done following the protocol from Macosko et al [@Macosko2015]. However, instead of using G1/S, S, G2/M, M, and M/G1 as the different phases, we used G1/S, S, G2, G2/M, and M/G1 following the original classification from Whitfield et al [@Whitfield2002].
To compare the results before and after applying our normalization method, we used the raw molecule counts (Figure `r fig_supp_cellcycle`A, C, and E) and the corrected molecule numbers (Figure `r fig_supp_cellcycle`B, D, and F) as the input data files for the classification.
The raw molecule counts were used to calculate the total gene molecule-counts for both classifications (Figure `r fig_supp_cellcycle`A and B).

### A framework for testing individual and batch effects

Individual effect and batch effect between the single cell samples were evaluated in a series of analyses that examine the potential sources of technical variation on gene expression measurements. These analyses took into consideration that in our study design, sources of variation between single cell samples naturally fall into a hierarchy of individuals and C1 batches. In these analyses, the variation introduced at the individual-level and at the batch-level were modeled in a nested framework that allows random noise between C1 batches within individuals. Specifically, for each cell sample in individual $i$, replicate $j$ and well $k$, we use $y_{ijk}$ to denote some sample measurement (e.g., total molecule-counts), and $x_{ijk}$ to denote the covariate, if any, that is associated with $y_{ijk}$ (e.g., sample RNA concentration measurement), and fit a linear mixed model with the fixed effect of individual $\alpha_i$ and the random effect of batch $b_{ij}$:
...
1. Total molecule-count for the endogenous genes was modeled as a function of the covariate sample RNA concentration, individual effect and batch effect.
We excluded individual NA19098 from the analysis, because sample concentration was quantified in only one of the two C1 replicates.
...
The goal here is to quantify proportion of cell-to-cell variance per gene due to individual effect and to batch, which is different from the previous section, which aims to test the statistical significance of individual effect and batch effect.
...
We used the blmer function in the R package blme and set the default penalty function which is the logarithm of a gamma density (shape parameter = 2 and rate parameter tends to zero).
...
Because these samples contained RNA molecules from ~10,000 cells, we could not assume that the 1,024 UMIs we added could accurately estimate the true number of molecules.
We standardized them to log~2~ cpm.

### Remove technical batch effect due to between-sample gene expression correlations within C1 replicates
...
where $y_{g,ijk}$ denotes log~2~ counts-per-million (cpm) of gene $g$ in individual $i$, replicate $j$, and cell $k$; $i = NA19098, NA19101, NA19239$, $j = 1, \dots, n_i$ with $n_i$ the number of replicates in individual $i$, $k = 1, \dots, n_{g,ij}$ with $n_{g,ij}$ the number of cells in individual $i$ replicate $j$. $\mu_g$ denote gene mean expression level across cells, $\alpha_{g,i}$ quantify individual effect in mean gene expression, $b_{g,ij}$ model replicate effect in mean expression level, which are assumed to be stochastic, independent, and identifically distributed with mean 0 and variance $\sigma^2_{g,b})$, finally $\epsilon_{g,ijk}$ describe residual variation in gene expression.
...
where $\widehat{b}_{g,ij}$ are the least-squares estimates. The computations in this step are done with the gls.series function of the limma package [@limma].

### Measurement of gene expression noise

While examining gene expression noise (using CV) as a function of RNA abundance (mean) across C1 replicates, we found that CV of molecule counts among endogenous genes and ERCC spike-in controls suggested similar expression variability patterns.
Both endogenous and ERCC spike-in control CV patterns track over-dispersed Poisson distribution (Figure `r fig_supp_CV`A-C), which is consistent with previous studies [@Islam2014; @Brennecke2013].
...
### Identification of differential noise genes
...
$$ MAD_{g} = Median_{i= 1,2,3} \left| \text{adj-CV}_{g,i} -  Median_{i= 1,2,3} ({\text{adj-CV}}_{g,i}) \right|. $$

Large values of $MAD_{g}$ suggest large deviation from the median of the adjusted CV values.
We computed permutation-based p-values for each gene based on `r format(sum(grepl("ENSG", rownames(molecules_filter))), big.mark = ",")` permutation - the number of genes included in the filtered and batch-corrected data set.
...
### Identification of individuals from single cell RNA-seq reads
...
## Acknowledgment
We thank Nicholas E. Banovich for the Yoruba iPSC lines.
We thank the University of Chicago Genomics Facility for performing Next Generation Sequencing.
We thank members of the Pritchard and Gilad laboratories for valuable discussions during the preparation of this manuscript.
