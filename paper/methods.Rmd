...
The capture efficiency and TRA-1-60 staining were then inspected using the EVOS FL Cell Imaging System (Thermo Fisher) (Supplementary Table `r table_collection`A).
...
The average yield of samples per C1 plate ranged from 1.26-1.88 ng per microliter (Supplementary Table `r table_collection`A).
...
The master mixes, one mix with all the bulk samples and nine mixes corresponding to the three replicates for the three individuals, were sequenced across four flowcells using a design aimed to minimize the introduction of technical batch effects (Supplementary Table `r table_collection`B).
...
In an initial analysis investigating the percentage of reads mapping to the ERCC spike-in controls, we observed that replicate 2 of individual NA19098 was a clear outlier (Supplementary Fig. `r fig_supp_qc`A-B).
...
We chose the above criteria based on the distribution of these metrics in the empty wells (the cutoff is the 95th percentile, Supplementary Fig. `r fig_supp_qc`D-F).
In addition, we observed that some wells classified as containing only one cell were clustered with multi-cell wells when plotting 1) the number of gene molecules versus the concentration of the samples (Supplementary Fig. `r fig_supp_lda`A-B), and 2) the read to molecule conversion efficiency (total molecule number divided by total read number) of endogenous genes versus that of ERCC (Supplementary Fig. `r fig_supp_lda`C-D).
...
Specifically, LDA was performed to classify wells into empty, one-cell, and two-cell using the discriminant functions of 1) sample concentration and the number of gene molecules (Supplementary Fig. `r fig_supp_lda`A-B) and 2) endogenous and ERCC gene read to molecule conversion efficiency (Supplementary Fig. `r fig_supp_lda`C-D).
...
Both endogenous and ERCC spike-in control CV patterns approximately followed an over-dispersed Poisson distribution (Supplementary Fig. `r fig_supp_CV`A-C), which is consistent with previous studies [@Islam2014; @Brennecke2013].
...
After this transformation step, CV no longer had a polynomial relationship with mean gene molecule count (Supplementary Fig. `r fig_supp_CV`D-E).
...
Using the option --best to return the best matching individual, we obtained 100% accuracy identifying the single cells of all three individuals (Supplementary Fig. `r fig_supp_design`B).
