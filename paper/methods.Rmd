...
Our last normalization step adjusts the transformed log2 gene expression levels for cell-to-cell correlation within each C1 plate (or replicate). 
The algorithm follows a method developed for adjusting within-replicate correlation in microarray data (limma) [@Smyth2005].
We assumed that for each gene $g$, cells that belong to the same C1 replicate, say $j$, are correlated, for batches $j = 1, \dots, 8$. 
The batch effect is specific to the C1 replicate and independent of biological variation across cell lines. 
