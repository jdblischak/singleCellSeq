...
To assess the relative contributions of individual and technical variation, we analyzed per-gene expression profiles and computed variance component estimates for the effects of individual and C1 batch (Figure `r fig_supp_variance`).
The goal here was to quantify the proportion of cell-to-cell variance due to individual effect and to C1 batch at the per-gene level. In the previous previous section, we tested the statistical significance of differences between individuals and batches. This section also applies a linear mixed model on a per-gene expression (log scale) with an additional random component of individual effect.
