...
### Calculate the number of ERCC spiked-in molecules
...
We quantified cell-to-cell heterogeneity in transcriptional expression as a function of transcript abundance. First, squared coefficents of variations (CVs) are computed for each cell line on the normalized, batch-corrected molecule counts. To account for the confounding effect of mean gene expression level on CVs, we follow the approach described in Kolodziejczyk et al., 2015 [@Kolodziejczyk2015]: 

1. Sort genes by mean gene expression levels across cell lines.

2. Compute squared CVs for each gene. 

3. Starting from the gene with the lowest gene expression level, for every sliding window of 50 genes, substract the log10 median squared CVs from the log10 squared CVs of each cell line, and the number of overlapping genes between windows is 25. The computation was performed with the rollapply function of the R zoo package [@Zeileis2005]. 
