...
Using single cell gene expression profiling, one can identify novel cell types, study regulatory robustness, and characterize inter-cell variability in response to external signals. 
...
As expected, we found that the major source of variation in the gene expression data was driven by biological variation (i.e. the differences between the three individuals), but we also observed substantial technical variation between the replicates of each individual.
In contrast to the common notion, we observed that the conversion rate of reads to molecules using the UMIs was impacted by both biological and technical variation, indicating that absolute UMI counts cannot assumed to be an unbiased estimator of gene expression levels.
