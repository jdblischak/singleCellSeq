...
The read and molecule counts are highly correlated for both the ERCC control spike-ins (`r cor(counts$molecules[counts$feature == "control"], counts$reads[counts$feature == "control"])`) and genes (`r cor(counts$molecules[counts$feature == "gene"], counts$reads[counts$feature == "gene"])`).

```{r reads-v-molecules}
ggplot(counts, aes(x = molecules, y = reads)) + geom_point() +
...
This is likely due to genes that are highly expressed and have multiple transcription start sites, which would make it possible to tag more than 1,024 molecules.
...
counts$reads_fc_med <- log2(counts$reads / median(counts$reads))
counts$molecules_fc_med <- log2(counts$molecules / median(counts$molecules))
ggplot(counts, aes(x = molecules_fc_med, y = reads_fc_med)) + geom_point() + facet_wrap(~ feature) + geom_abline(intercept = 0, slope = 1, col = "red") + labs(title = "Reads and molecules diverge at lower expression levels", x = "Log2 fold change of molecule counts to median", y = "Log2 fold change of read counts to median")
```
